# 075 - IP Whitelisting & API Key Permission Scoping (Read, Trade, Withdraw)

## Purpose
Empower institutional traders and bot operators to securely access the exchange via API keys with strict granular permission scopes, CIDR IP address whitelisting, and mandatory key expiration.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for IP Whitelisting & API Key Permission Scoping (Read, Trade, Withdraw) across the Growww / NBSE platform.

## What You Are Building
An API key management service allowing users to create API keypairs (API Key and HMAC-SHA256 Secret), configure granular permission scopes (Read-Only, Spot Trading, Withdrawals), bind specific IPv4/IPv6 addresses, and rotate keys.
...
## Scope Boundaries
- **In Scope:**
- Granular permission scoping: READ (market/account data), TRADE (place/cancel spot orders), WITHDRAW (crypto transfers).
- Strict IP address whitelisting: rejects requests from non-whitelisted IPs even with valid cryptographic signatures.
- HMAC-SHA256 request signing: validates timestamp, nonce, method, path, and payload against API secret.
- Automated 90-day inactivity expiration and one-click emergency revocation of all active API keys.
- **Out of Scope / Handled Elsewhere:**
- Retail biometric UI sheets.
- Physical cash handling.

## Technology to Use
- Core Technologies: Go API Auth middleware, Redis IP whitelist cache, PostgreSQL encrypted secrets (AES-256-GCM), HMAC validator.
...
- API Gateway, Auth Service, Institutional Account Service, Security Dashboard, Developer Portal.

## Blockchain Interaction
API keys permitted for trading must hold delegated EIP-712 session signing authorization on Hyperledger Besu.
...
1. Review system architectural boundaries and regulatory invariants for IP Whitelisting & API Key Permission Scoping (Read, Trade, Withdraw).
2. Define the core domain entities, data models, and state lifecycle transitions for IP Whitelisting & API Key Permission Scoping (Read, Trade, Withdraw).
...
package growww.api.key.v1;

option go_package = "growww/packages/proto/growww/api/key/v1;keyv1";
...
## Acceptance Criteria
- [ ] Core specification and domain data models for IP Whitelisting & API Key Permission Scoping (Read, Trade, Withdraw) fully documented.
...
- **Regulatory Framework:** SEBI circulars on Algorithmic and Automated Trading API security controls, OWASP API Security Standard.
