# 089 - GIFT City IFSCA Offshore Trading Window & Multi-Currency Clearing

## Purpose
Unlock global institutional capital by engineering a dedicated offshore international trading window operating from Gujarat International Finance Tec-City (GIFT City) under IFSCA regulatory jurisdiction.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for GIFT City IFSCA Offshore Trading Window & Multi-Currency Clearing across the Growww / NBSE platform.

## What You Are Building
A multi-currency offshore exchange gateway enabling Non-Resident Indians (NRIs) and global institutions to trade cryptocurrency spot and derivatives in US Dollars (USD) with zero Indian domestic currency restrictions.
...
## Scope Boundaries
- **In Scope:**
- Multi-currency settlement: native support for USD, EUR, AED, and USDT without mandatory INR conversion.
- IFSCA regulatory sandbox compliance: segregated offshore entity infrastructure located physically in GIFT City.
- Cross-border tax exemptions: zero zero on-chain TDS under Section 194S and zero capital gains tax for offshore entities in GIFT City.
- Liberalised Remittance Scheme (LRS) integration for resident Indians funding offshore accounts up to $250,000/year.
- **Out of Scope / Handled Elsewhere:**
- Domestic Indian cash handling.
- Local municipal tax filings.

## Technology to Use
- Core Technologies: Go offshore gateway, Swift MT/MX ISO-20022 banking connector, Multi-Currency Ledger, GIFT City Cloud PoP.
...
- GIFT City Gateway, Offshore KYC Service, Multi-Currency Treasury, IFSCA Reporting Service, Institutional Portal.

## Blockchain Interaction
Maintains a dedicated GIFT City permissioned partition on Hyperledger Besu settling offshore USD and USDT transactions.
...
1. Review system architectural boundaries and regulatory invariants for GIFT City IFSCA Offshore Trading Window & Multi-Currency Clearing.
2. Define the core domain entities, data models, and state lifecycle transitions for GIFT City IFSCA Offshore Trading Window & Multi-Currency Clearing.
...
package growww.giftcity.v1;

option go_package = "growww/packages/proto/growww/giftcity/v1;giftcityv1";

message ProcessOffshoreTradeRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ProcessOffshoreTradeResponse {
  string request_id = 1;
  bool success = 2;
...
message ProcessOffshoreTradeEvent {
  string event_id = 1;
...
service GIFTCityGatewayService {
  rpc ProcessOffshoreTrade(ProcessOffshoreTradeRequest) returns (ProcessOffshoreTradeResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for GIFT City IFSCA Offshore Trading Window & Multi-Currency Clearing fully documented.
...
- **Regulatory Framework:** IFSCA (Capital Market Intermediaries) Regulations 2021, RBI Liberalised Remittance Scheme (LRS) Guidelines.
