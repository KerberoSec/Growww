# 025 - One-Cancels-the-Other (OCO) Order Orchestrator

## Purpose
Allow spot traders to formulate sophisticated bracket trading strategies by pairing a limit order with a stop-loss order under an atomic One-Cancels-the-Other (OCO) execution contract.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for One-Cancels-the-Other (OCO) Order Orchestrator across the Growww / NBSE platform.

## What You Are Building
An OCO orchestration coordinator that couples a resting limit maker order and a conditional stop trigger order, guaranteeing that the execution or partial fill of one leg immediately and atomically cancels the remaining leg without double balance allocation.
...
## Scope Boundaries
- **In Scope:**
- Atomic dual-order binding: Limit Leg (take profit) and Stop-Limit/Stop-Market Leg (stop loss).
- Shared balance reservation: single collateral pool locked for both orders preventing double-spend.
- Immediate sub-millisecond cancellation of sibling order upon first-leg trigger or execution.
- State synchronization ensuring partial fills proportionally scale or lock remaining contingent legs.
- **Out of Scope / Handled Elsewhere:**
- Perpetual futures leverage margin.
- Fiat bank deposits.

## Technology to Use
- Core Technologies: Go coordination service, Redis transactional multi-exec scripts, Kafka state transition event bus.
...
- Order Orchestrator, Matching Engine, Conditional Order Daemon, Ledger Service, WebSocket Client Gateway.

## Blockchain Interaction
Ensures single token balance lock on-chain and releases unspent reservations on Hyperledger Besu upon cancellation.
...
1. Review system architectural boundaries and regulatory invariants for One-Cancels-the-Other (OCO) Order Orchestrator.
2. Define the core domain entities, data models, and state lifecycle transitions for One-Cancels-the-Other (OCO) Order Orchestrator.
...
package growww.order.oco.v1;

option go_package = "growww/packages/proto/growww/order/oco/v1;ocov1";
...
## Acceptance Criteria
- [ ] Core specification and domain data models for One-Cancels-the-Other (OCO) Order Orchestrator fully documented.
...
- **Regulatory Framework:** IOSCO Retail Market Conduct Principles, SEBI Automated Order Routing rules.
