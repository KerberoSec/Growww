# 018 - Merkle Proof of Solvency & Cryptographic Proof of Reserves (PoR/PoL)

## Purpose
Provide absolute, mathematically verifiable transparency to retail and institutional traders by publishing regular cryptographic Proof of Reserves and zero-knowledge Proof of Liabilities, proving 100%+ backing of all customer balances.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Merkle Proof of Solvency & Cryptographic Proof of Reserves (PoR/PoL) across the Growww / NBSE platform.

## What You Are Building
A high-performance Sparse Merkle Tree (SMT) and zk-SNARK solvency verification pipeline that aggregates user account balances into an anonymized Merkle liability tree, cross-referenced with verifiable on-chain asset reserves.
...
## Scope Boundaries
- **In Scope:**
- Sparse Merkle Tree generation calculating root hash of all user balances (BTC, USDT, INR cash).
- User self-verification interface allowing any trader to verify their balance inclusion using a Merkle leaf proof.
- On-chain Proof of Reserve registry smart contract on Hyperledger Besu updating hourly.
- Automated solvency health checks: alert triggered if Reserve/Liability ratio drops below 102%.
- **Out of Scope / Handled Elsewhere:**
- Internal risk margin calculation.
- Relational database transactional ledger.

## Technology to Use
- Core Technologies: Sparse Merkle Tree in Go, Circom / Groth16 zk-SNARK circuits, PostgreSQL liability snapshot, Hyperledger Besu SolvencyRegistry.sol.
...
- Proof of Solvency Daemon, Ledger Service, Marketing Web Portal, Auditor Verification Node, Besu Smart Contract.

## Blockchain Interaction
Besu SolvencyRegistry.sol storing hourly Merkle root commitments and third-party auditor signatures.
...
1. Review system architectural boundaries and regulatory invariants for Merkle Proof of Solvency & Cryptographic Proof of Reserves (PoR/PoL).
2. Define the core domain entities, data models, and state lifecycle transitions for Merkle Proof of Solvency & Cryptographic Proof of Reserves (PoR/PoL).
...
package growww.solvency.v1;

option go_package = "growww/packages/proto/growww/solvency/v1;solvencyv1";

message UserProofRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
service SolvencyVerificationService {
  rpc GetUserProof(UserProofRequest) returns (UserProofResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Merkle Proof of Solvency & Cryptographic Proof of Reserves (PoR/PoL) fully documented.
...
- **Regulatory Framework:** SEBI Mandate on Client Fund Segregation, FIU-IND asset auditing norms, Statutory Proof of Reserve disclosures.
