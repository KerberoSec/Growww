# 060 - Gamified Trade Streaks, Achievements & Social Referral Badges

## Purpose
Boost platform virality, daily active usage (DAU), and community retention through a clean, compliant gamification layer celebrating disciplined trading milestones, daily check-in streaks, and referral achievement badges.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Gamified Trade Streaks, Achievements & Social Referral Badges across the Growww / NBSE platform.

## What You Are Building
An engagement and reward tracking system that monitors user milestones (e.g., 7-Day Trading Streak, First Stop-Loss Used, Crypto Pioneer), unlocks tiered profile badges, and calculates transparent referral fee-share percentages.
...
## Scope Boundaries
- **In Scope:**
- Trading streak counter tracking consecutive days of market activity or educational article completions.
- Disciplined trader achievement badges: rewards for setting stop-losses, completing risk quizzes, and verifying KYC.
- Referral tier progression: Bronze (10% fee share), Silver (20%), Gold (30%), Diamond (40%) with dynamic unlock criteria.
- Social sharing cards: beautifully formatted dark-theme PnL and badge achievement image generator for social sharing.
- **Out of Scope / Handled Elsewhere:**
- Direct cash gambling mechanics.
- Unregistered securities promotion.

## Technology to Use
- Core Technologies: Go gamification daemon, Redis streak counter, PostgreSQL user achievements, Puppeteer social card renderer.
...
- Gamification Service, Referral Service, User Profile Service, Notification Service, Mobile & Web Frontends.

## Blockchain Interaction
Optionally mints non-transferable Soulbound Tokens (SBTs) on Hyperledger Besu for verified trading achievements.
...
1. Review system architectural boundaries and regulatory invariants for Gamified Trade Streaks, Achievements & Social Referral Badges.
2. Define the core domain entities, data models, and state lifecycle transitions for Gamified Trade Streaks, Achievements & Social Referral Badges.
...
package growww.gamification.v1;

option go_package = "growww/packages/proto/growww/gamification/v1;gamificationv1";

message UserAchievementsRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
service GamificationService {
  rpc GetUserAchievements(UserAchievementsRequest) returns (UserAchievementsResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Gamified Trade Streaks, Achievements & Social Referral Badges fully documented.
...
- **Regulatory Framework:** ASCI (Advertising Standards Council of India) Guidelines on Influencer and Referral Disclosures in Financial Services.
