# 094 - Database Point-In-Time Recovery (PITR) & CDC Debezium Pipeline

## Purpose
Ensure continuous auditability and instant recovery from catastrophic human error or data corruption by deploying a Change Data Capture (CDC) pipeline with Debezium and 35-day Point-In-Time Recovery (PITR).

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Database Point-In-Time Recovery (PITR) & CDC Debezium Pipeline across the Growww / NBSE platform.

## What You Are Building
A continuous data capture and backup architecture deploying Debezium CDC on PostgreSQL write-ahead logs (WAL), streaming real-time row-level mutations into Kafka, backed by continuous S3 WAL archiving for point-in-time recovery.
...
## Scope Boundaries
- **In Scope:**
- Debezium CDC connector streaming every database insert, update, and delete into Kafka with microsecond latency.
- Continuous PostgreSQL WAL archiving to AWS S3 Glacier with automated point-in-time recovery (PITR) capability.
- Zero-downtime database restore drills tested weekly: restores complete 10TB database to exact second.
- Immutable audit log generation capturing every state mutation for forensic reconstruction.
- **Out of Scope / Handled Elsewhere:**
- Frontend animation curves.
- Demo virtual token minting.

## Technology to Use
- Core Technologies: Debezium 2.5+, PostgreSQL 16 wal2json, Apache Kafka, pgBackRest, AWS S3 Object Lock.
...
- Database Engineering, Data Platform Team, Kafka Event Streaming, Compliance Audit Store, Backup Automation.

## Blockchain Interaction
CDC pipeline streams on-chain settlement receipts from Besu nodes into relational search indices for instant lookup.
...
1. Review system architectural boundaries and regulatory invariants for Database Point-In-Time Recovery (PITR) & CDC Debezium Pipeline.
2. Define the core domain entities, data models, and state lifecycle transitions for Database Point-In-Time Recovery (PITR) & CDC Debezium Pipeline.
...
package growww.db.cdc.v1;

option go_package = "growww/packages/proto/growww/db/cdc/v1;cdcv1";

message StreamDatabaseMutationsRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message StreamDatabaseMutationsResponse {
  string request_id = 1;
  bool success = 2;
...
message StreamDatabaseMutationsEvent {
  string event_id = 1;
...
service DataCaptureService {
  rpc StreamDatabaseMutations(StreamDatabaseMutationsRequest) returns (StreamDatabaseMutationsResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Database Point-In-Time Recovery (PITR) & CDC Debezium Pipeline fully documented.
...
- **Regulatory Framework:** SEBI Cyber Security Framework norms on database backup integrity and immutable audit storage retention.
