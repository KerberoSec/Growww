# 072 - Two-Factor Authentication (TOTP, WebAuthn/Passkeys, YubiKey FIDO2)

## Purpose
Provide ironclad defense against credential stuffing and phishing attacks by implementing modern Two-Factor Authentication supporting RFC-6238 TOTP apps, WebAuthn Passkeys, and hardware FIDO2 YubiKeys.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Two-Factor Authentication (TOTP, WebAuthn/Passkeys, YubiKey FIDO2) across the Growww / NBSE platform.

## What You Are Building
An enterprise multi-factor authentication service supporting Google Authenticator / Authy TOTP, Apple/Google biometric Passkeys (WebAuthn), and hardware security keys (FIDO2 / U2F), complete with cryptographic backup recovery codes.
...
## Scope Boundaries
- **In Scope:**
- Standard RFC-6238 Time-Based One-Time Password (TOTP) generation and verification with QR code pairing.
- FIDO2 / WebAuthn Passkeys implementation enabling passwordless biometric logins on mobile and web.
- Hardware security key support (YubiKey 5 Series) with WebAuthn browser attestation verification.
- Cryptographically generated 16-character backup recovery codes stored as bcrypt hashes in PostgreSQL.
- **Out of Scope / Handled Elsewhere:**
- Orderbook matching algorithms.
- TradingView chart indicators.

## Technology to Use
- Core Technologies: Go Auth Service, duo-labs/webauthn library, pquerna/otp library, PostgreSQL credentials store, Flutter local_auth.
...
- Auth Service, API Gateway, Security Center UI, Mobile Keyring, User Account Service.

## Blockchain Interaction
Requires 2FA verification before permitting sensitive smart contract interactions or withdrawal approvals.
...
1. Review system architectural boundaries and regulatory invariants for Two-Factor Authentication (TOTP, WebAuthn/Passkeys, YubiKey FIDO2).
2. Define the core domain entities, data models, and state lifecycle transitions for Two-Factor Authentication (TOTP, WebAuthn/Passkeys, YubiKey FIDO2).
...
package growww.auth.twofactor.v1;

option go_package = "growww/packages/proto/growww/auth/twofactor/v1;twofactorv1";
...
message PasskeyAuthenticationEvent {
  string event_id = 1;
...
service TwoFactorAuthService {
  rpc VerifyPasskeyAuthentication(PasskeyAuthenticationRequest) returns (PasskeyAuthenticationResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Two-Factor Authentication (TOTP, WebAuthn/Passkeys, YubiKey FIDO2) fully documented.
...
- **Regulatory Framework:** SEBI Master Circular on mandatory Two-Factor Authentication for internet-based trading systems.
