# 031 - Live Market Price Mirroring Pipeline for Demo Trading

## Purpose
Deliver an authentic, zero-cost paper trading experience by mirroring live external Bitcoin (BTC/USDT) and crypto spot prices in real-time into the demo trading sandbox with sub-100ms latency.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Live Market Price Mirroring Pipeline for Demo Trading across the Growww / NBSE platform.

## What You Are Building
A streaming price mirroring pipeline that ingests live market trades, orderbook depth, and ticker updates from Binance and Coinbase Pro, normalizing them into the Growww demo market data feed.
...
## Scope Boundaries
- **In Scope:**
- High-frequency WebSocket connectors to Binance and Coinbase Pro BTC/USDT spot endpoints.
- Sub-100ms price tick normalization and ingestion into demo orderbook simulation bus.
- Heartbeat monitoring with automatic reconnection and fallback across redundant market data feeds.
- Zero impact on production trading infrastructure via strictly isolated Kafka message topics.
- **Out of Scope / Handled Elsewhere:**
- Real-money settlement smart contracts.
- User KYC biometric verification.

## Technology to Use
- Core Technologies: Go Gorilla WebSocket client, Kafka dedicated demo topics, Redis cluster for demo quote caching, Prometheus metrics.
...
- Demo Feed Ingress Service, Demo Matching Engine, Market Data Gateway, Flutter Demo Client, Web Demo Terminal.

## Blockchain Interaction
Does not write to Besu mainnet; records simulated ticks to Besu Testnet Oracle contract for on-chain demo verification.
...
1. Review system architectural boundaries and regulatory invariants for Live Market Price Mirroring Pipeline for Demo Trading.
2. Define the core domain entities, data models, and state lifecycle transitions for Live Market Price Mirroring Pipeline for Demo Trading.
...
package growww.demo.market.v1;

option go_package = "growww/packages/proto/growww/demo/market/v1;marketv1";

message StreamDemoTicksRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message StreamDemoTicksResponse {
  string request_id = 1;
  bool success = 2;
...
service DemoPriceMirrorService {
  rpc StreamDemoTicks(StreamDemoTicksRequest) returns (StreamDemoTicksResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Live Market Price Mirroring Pipeline for Demo Trading fully documented.
...
- **Regulatory Framework:** SEBI Investor Education and Protection Fund (IEPF) guidelines for financial market simulation tools.
