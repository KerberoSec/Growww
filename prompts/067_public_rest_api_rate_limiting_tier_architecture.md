# 067 - Public REST API & Rate-Limiting Tier Architecture (Leaky Bucket)

## Purpose
Expose a developer-friendly, high-performance public REST API for algorithmic traders and external aggregators (CoinMarketCap, CoinGecko) while protecting backend services through multi-tiered leaky bucket rate limiting.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Public REST API & Rate-Limiting Tier Architecture (Leaky Bucket) across the Growww / NBSE platform.

## What You Are Building
A public REST API gateway implementing OpenAPI 3.0 standards, JSON responses, API key authentication, and distributed Redis-backed leaky bucket rate limiting categorized into Public, Authenticated, and VIP tiers.
...
## Scope Boundaries
- **In Scope:**
- Comprehensive REST endpoints for Market Data, Order Management, Account Balances, and Blockchain Settlement Status.
- Multi-tiered rate limits: Public (20 req/s per IP), Authenticated Retail (50 req/s), VIP Institutional (200 req/s).
- Standard HTTP 429 Too Many Requests response with RFC-6585 Retry-After and X-RateLimit-* headers.
- Distributed rate-limiting implementation using Redis cell / leaky bucket scripts ensuring sub-millisecond overhead.
- **Out of Scope / Handled Elsewhere:**
- Internal database migrations.
- Depository pledge operations.

## Technology to Use
- Core Technologies: Go Gin / Fiber REST framework, Redis leaky-bucket scripts, Envoy Proxy rate-limit filter, OpenAPI v3.
...
- Public API Gateway, Auth Service, Rate Limiting Service, Redis Cluster, External Developer Community.

## Blockchain Interaction
Exposes read-only REST endpoints allowing developers to query Hyperledger Besu transaction receipts and smart contract addresses.
...
1. Review system architectural boundaries and regulatory invariants for Public REST API & Rate-Limiting Tier Architecture (Leaky Bucket).
2. Define the core domain entities, data models, and state lifecycle transitions for Public REST API & Rate-Limiting Tier Architecture (Leaky Bucket).
...
package growww.api.gateway.v1;

option go_package = "growww/packages/proto/growww/api/gateway/v1;gatewayv1";
...
service PublicAPIGatewayService {
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Public REST API & Rate-Limiting Tier Architecture (Leaky Bucket) fully documented.
...
- **Regulatory Framework:** SEBI guidelines on algorithmic API access, OWASP API Security Top 10 compliance.
