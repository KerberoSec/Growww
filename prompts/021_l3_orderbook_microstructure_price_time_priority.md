# 021 - L3 In-Memory Orderbook Microstructure & Price-Time Priority

## Purpose
Achieve microsecond-latency deterministic trade matching for spot cryptocurrency pairs (BTC/USDT) by implementing a lock-free, single-writer in-memory Level 3 orderbook with strict price-time priority.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for L3 In-Memory Orderbook Microstructure & Price-Time Priority across the Growww / NBSE platform.

## What You Are Building
The core Rust matching engine orderbook specification utilizing intrusive doubly linked lists, BTreeMap price levels, ring-buffer event sequencing, and zero-allocation memory pools for microsecond execution.
...
## Scope Boundaries
- **In Scope:**
- Level 3 orderbook tracking individual order IDs, sizes, and microsecond ingress timestamps at every tick level.
- Strict Price-Time Priority (FIFO) matching algorithm for buy and sell limit orders.
- Lock-free single-writer thread pinned to dedicated CPU core via NUMA-aware thread affinity.
- Real-time generation of Level 2 depth diffs and trade execution reports.
- **Out of Scope / Handled Elsewhere:**
- User authentication and session tokens.
- Database transaction logging.

## Technology to Use
- Core Technologies: Rust 1.76, crossbeam lock-free channels, intrusive-collections, criterion benchmarking, jemalloc memory allocator.
...
- Matching Engine Core, Sequencer Daemon, Market Data Broadcaster, Trade Settlement Relayer, Redis Cache.

## Blockchain Interaction
Batches matched trade execution reports and streams them to Hyperledger Besu DvPAtomicSettlement.sol for atomic settlement.
...
1. Review system architectural boundaries and regulatory invariants for L3 In-Memory Orderbook Microstructure & Price-Time Priority.
2. Define the core domain entities, data models, and state lifecycle transitions for L3 In-Memory Orderbook Microstructure & Price-Time Priority.
...
option go_package = "growww/packages/proto/growww/matching/v1;matchingv1";
...
## Acceptance Criteria
- [ ] Core specification and domain data models for L3 In-Memory Orderbook Microstructure & Price-Time Priority fully documented.
...
- **Regulatory Framework:** SEBI algorithmic trading circulars on order queue fairness, IOSCO Principles for Financial Markets Infrastructure.
