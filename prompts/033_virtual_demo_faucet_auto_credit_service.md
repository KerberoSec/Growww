# 033 - Virtual Demo Faucet Service (10,000 USDT & 1 BTC Auto-Credit)

## Purpose
Empower new users to start demo trading immediately upon signup by providing an automated, rate-limited virtual faucet that dispenses 10,000 virtual USDT and 1 virtual BTC with instant reset capabilities.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Virtual Demo Faucet Service (10,000 USDT & 1 BTC Auto-Credit) across the Growww / NBSE platform.

## What You Are Building
A virtual faucet service and smart contract that credits paper trading accounts with initial sandbox funds, provides a one-click balance reset button, and enforces anti-abuse rate limits per device.
...
## Scope Boundaries
- **In Scope:**
- Automatic 10,000 virtual USDT and 1 virtual BTC balance provisioning upon activating demo mode.
- One-click reset functionality allowing traders to restore starting balances anytime.
- Anti-spam rate limiting: maximum 3 resets per 24-hour period per device fingerprint.
- Virtual token ledger maintaining isolated double-entry journals for all faucet transactions.
- **Out of Scope / Handled Elsewhere:**
- Real cryptocurrency wallet addresses.
- Statutory tax deduction calculations.

## Technology to Use
- Core Technologies: Go microservice, Redis rate-limiting leaky bucket, PostgreSQL virtual ledger, Flutter UI integration.
...
- Demo Faucet Service, Demo Ledger Service, User Management Service, Mobile Client, Web Terminal.

## Blockchain Interaction
Mints virtual ERC-20 testnet tokens (vUSDT and vBTC) on Hyperledger Besu Testnet for educational transparency.
...
1. Review system architectural boundaries and regulatory invariants for Virtual Demo Faucet Service (10,000 USDT & 1 BTC Auto-Credit).
2. Define the core domain entities, data models, and state lifecycle transitions for Virtual Demo Faucet Service (10,000 USDT & 1 BTC Auto-Credit).
...
package growww.demo.faucet.v1;

option go_package = "growww/packages/proto/growww/demo/faucet/v1;faucetv1";

message ClaimDemoFundsRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ClaimDemoFundsResponse {
  string request_id = 1;
  bool success = 2;
...
service DemoFaucetService {
  rpc ClaimDemoFunds(ClaimDemoFundsRequest) returns (ClaimDemoFundsResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Virtual Demo Faucet Service (10,000 USDT & 1 BTC Auto-Credit) fully documented.
...
- **Regulatory Framework:** Consumer Protection Act 2019 fair representation standards for financial trading simulators.
