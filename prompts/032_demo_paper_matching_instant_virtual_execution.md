# 032 - Demo Paper Matching Engine & Instant Virtual Order Execution

## Purpose
Provide beginners and strategy backtesters with an instant-execution simulated trading environment that mirrors live crypto orderbook dynamics without risking actual financial capital.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Demo Paper Matching Engine & Instant Virtual Order Execution across the Growww / NBSE platform.

## What You Are Building
A standalone in-memory paper matching engine that executes simulated limit, market, and stop orders against real-time market data ticks, tracking virtual portfolio balances and realistic fill probabilities.
...
## Scope Boundaries
- **In Scope:**
- Independent in-memory orderbook executing against mirrored real-time market price levels.
- Realistic fill simulation: limit orders only execute when market depth crosses order price.
- Instant virtual order placement and cancellation with zero transaction or gas fees.
- Complete isolation of demo state from real-money user ledger balances.
- **Out of Scope / Handled Elsewhere:**
- Real-money bank withdrawals.
- Hardware Security Module signing.

## Technology to Use
- Core Technologies: Rust in-memory orderbook simulator, Redis for virtual account balance state, gRPC demo gateway.
...
- Demo Matching Engine, Demo Account Service, Market Data Mirror, Flutter App Demo Screen, Web Demo Portal.

## Blockchain Interaction
Executes simulated transactions on Hyperledger Besu Testnet or an in-memory virtual state machine.
...
1. Review system architectural boundaries and regulatory invariants for Demo Paper Matching Engine & Instant Virtual Order Execution.
2. Define the core domain entities, data models, and state lifecycle transitions for Demo Paper Matching Engine & Instant Virtual Order Execution.
...
package growww.demo.matching.v1;

option go_package = "growww/packages/proto/growww/demo/matching/v1;matchingv1";
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Demo Paper Matching Engine & Instant Virtual Order Execution fully documented.
...
- **Regulatory Framework:** SEBI Sandbox guidelines on non-risk simulated trading environments for retail investors.
