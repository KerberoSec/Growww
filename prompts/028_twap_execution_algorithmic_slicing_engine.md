# 028 - Time-Weighted Average Price (TWAP) Execution Algorithm

## Purpose
Allow institutional traders to execute large-scale spot crypto acquisitions or divestments evenly over specified time windows, minimizing market impact and achieving the Time-Weighted Average Price benchmark.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Time-Weighted Average Price (TWAP) Execution Algorithm across the Growww / NBSE platform.

## What You Are Building
A TWAP execution service that divides large parent spot orders into discrete, evenly distributed child slices executed at regular time intervals, with dynamic price limit adjustments based on real-time spread and volume.
...
## Scope Boundaries
- **In Scope:**
- User-configurable parameters: total quantity, total duration (e.g., 60 minutes), slice frequency, and limit price cap.
- Randomized time jitter (+/- 20% interval) and size variance to camouflage algorithmic execution.
- Real-time TWAP vs VWAP performance tracking against exchange benchmark index.
- Automated pause and notification if market spread widens beyond configured risk parameters.
- **Out of Scope / Handled Elsewhere:**
- Retail manual spot orders.
- Cold vault sweeping.

## Technology to Use
- Core Technologies: Go algorithmic engine, Temporal.io workflow orchestrator for reliable timed executions, PostgreSQL for slice logs.
...
- Algo Execution Service, Matching Engine, Risk Surveillance Daemon, FIX Protocol Gateway.

## Blockchain Interaction
Individual matched child slices settle on Hyperledger Besu; parent performance report archived in audit store.
...
1. Review system architectural boundaries and regulatory invariants for Time-Weighted Average Price (TWAP) Execution Algorithm.
2. Define the core domain entities, data models, and state lifecycle transitions for Time-Weighted Average Price (TWAP) Execution Algorithm.
...
package growww.order.twap.v1;

option go_package = "growww/packages/proto/growww/order/twap/v1;twapv1";

message StartTWAPSessionRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message StartTWAPSessionResponse {
  string request_id = 1;
  bool success = 2;
...
service TWAPService {
  rpc StartTWAPSession(StartTWAPSessionRequest) returns (StartTWAPSessionResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Time-Weighted Average Price (TWAP) Execution Algorithm fully documented.
...
- **Regulatory Framework:** SEBI Institutional Block Trading norms, CFA Institute Trade Allocation Guidelines.
