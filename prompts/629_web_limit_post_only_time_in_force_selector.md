# 629 - Web Limit Order Entry with Post-Only & Time-in-Force Rules

## Purpose
Deliver an institutional-grade, highly performant web interface specification for Web Limit Order Entry with Post-Only & Time-in-Force Rules on Next.js 14, providing ultra-low latency, fluid keyboard interactions, and responsive multi-monitor trading layouts.

This document establishes the authoritative technical blueprint, visual UI layout, interactive gestures, state management architecture, failure modes, and acceptance criteria for Web Limit Order Entry with Post-Only & Time-in-Force Rules across the Growww / NBSE trading platform.

## What You Are Building
Pro limit order form with Post-Only guarantee checkbox and GTC, IOC, and FOK execution instruction selectors.
...
1. Review UX design mockups and web interaction requirements for Web Limit Order Entry with Post-Only & Time-in-Force Rules.
...
message WeblimitpostonlytimeinforceselectorState {
  string user_id = 1;
...
service WeblimitpostonlytimeinforceselectorService {
  rpc UpdateViewState(WeblimitpostonlytimeinforceselectorState) returns (WeblimitpostonlytimeinforceselectorState);
}
```
...
## Acceptance Criteria
- [ ] Complete Next.js 14 UI layout, wireframe, and component architecture for Web Limit Order Entry with Post-Only & Time-in-Force Rules fully specified.
