# 035 - Seamless Single-Click Demo/Real Mode Switcher Architecture

## Purpose
Deliver an intuitive, friction-free user experience allowing traders to toggle between Demo (Paper) and Real-Money trading modes in a single click with instant visual feedback and distinct theme accents.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Seamless Single-Click Demo/Real Mode Switcher Architecture across the Growww / NBSE platform.

## What You Are Building
A client-side and backend mode-switching architecture that updates application state instantly, swaps active WebSocket subscription streams, and switches UI visual accents (Golden Amber for Demo vs Emerald Neon for Real).
...
## Scope Boundaries
- **In Scope:**
- Instantaneous client-side state toggle without requiring app restart or re-login.
- Dynamic WebSocket feed re-subscription: switches from live real book to mirrored demo book in <200ms.
- Unmistakable visual UI indicator: persistent top-bar banner displaying 'DEMO TRADING - VIRTUAL FUNDS'.
- Session context validation ensuring trade orders are never routed to the wrong environment.
- **Out of Scope / Handled Elsewhere:**
- Hardware root key generation.
- Off-chain orderbook matching.

## Technology to Use
- Core Technologies: Flutter Riverpod state management, Next.js Zustand store, WebSocket channel multiplexer, Go session validator.
...
- API Gateway, Session Management Service, Mobile Flutter Client, Web Trading Terminal, Redis Session Store.

## Blockchain Interaction
Directs client transaction dispatch to either Besu Testnet or Besu Mainnet based on authenticated session context.
...
1. Review system architectural boundaries and regulatory invariants for Seamless Single-Click Demo/Real Mode Switcher Architecture.
2. Define the core domain entities, data models, and state lifecycle transitions for Seamless Single-Click Demo/Real Mode Switcher Architecture.
...
package growww.ui.mode.v1;

option go_package = "growww/packages/proto/growww/ui/mode/v1;modev1";

message SwitchTradingModeRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message SwitchTradingModeResponse {
  string request_id = 1;
  bool success = 2;
...
message SwitchTradingModeEvent {
  string event_id = 1;
...
service ModeSwitchService {
  rpc SwitchTradingMode(SwitchTradingModeRequest) returns (SwitchTradingModeResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Seamless Single-Click Demo/Real Mode Switcher Architecture fully documented.
...
- **Regulatory Framework:** SEBI guidelines on clear demarcation between real and simulated financial instruments to prevent user confusion.
