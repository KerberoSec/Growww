# 062 - FIX Protocol 5.0 SP2 Gateway for Institutional Market Makers

## Purpose
Attract tier-1 institutional market makers, proprietary trading firms, and hedge funds by providing standardized, ultra-low-latency Financial Information eXchange (FIX 5.0 SP2) protocol connectivity.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for FIX Protocol 5.0 SP2 Gateway for Institutional Market Makers across the Growww / NBSE platform.

## What You Are Building
An enterprise FIX engine supporting FIX 5.0 SP2 messaging standards for NewOrderSingle (MsgType D), OrderCancelRequest (MsgType F), ExecutionReport (MsgType 8), and MarketDataSnapshotFullRefresh (MsgType W).
...
## Scope Boundaries
- **In Scope:**
- Full FIX 5.0 SP2 specification compliance with custom Growww user tags (Tag 20000+ for crypto asset IDs).
- Sub-100 microsecond message parsing and serialization using zero-copy binary and ASCII buffers.
- Session-level authentication via CompID, TargetCompID, HMAC-SHA256 signatures, and IP whitelisting.
- Drop-copy execution feed (MsgType 8) enabling real-time risk surveillance for institutional back-offices.
- **Out of Scope / Handled Elsewhere:**
- Retail mobile push notifications.
- Social gamification leaderboards.

## Technology to Use
- Core Technologies: QuickFIX/Go or C++ QuickFIX, DPDK kernel bypass, Linux epoll, raw TCP sockets, Prometheus metrics.
...
- FIX Gateway Cluster, Matching Engine Sequencer, Institutional Account Service, Risk Gateway.

## Blockchain Interaction
Settles institutional FIX trades through the same atomic Hyperledger Besu DvP pipeline with bulk clearing discounts.
...
1. Review system architectural boundaries and regulatory invariants for FIX Protocol 5.0 SP2 Gateway for Institutional Market Makers.
2. Define the core domain entities, data models, and state lifecycle transitions for FIX Protocol 5.0 SP2 Gateway for Institutional Market Makers.
...
package growww.fix.v1;

option go_package = "growww/packages/proto/growww/fix/v1;fixv1";

message SendExecutionReportRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message SendExecutionReportResponse {
  string request_id = 1;
  bool success = 2;
...
message SendExecutionReportEvent {
  string event_id = 1;
...
service FIXGatewayService {
  rpc SendExecutionReport(SendExecutionReportRequest) returns (SendExecutionReportResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for FIX Protocol 5.0 SP2 Gateway for Institutional Market Makers fully documented.
...
- **Regulatory Framework:** FIA (Futures Industry Association) Standards for Automated Trading, FIX Trading Community Guidelines.
