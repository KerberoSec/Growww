# 042 - Trading Hours Harmonization: 24/7 Crypto vs 09:15-15:30 IST Equity

## Purpose
Harmonize the operational interface and risk management rules between continuous 24/7/365 cryptocurrency spot trading and the scheduled Indian stock market session (09:15 to 15:30 IST).

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Trading Hours Harmonization: 24/7 Crypto vs 09:15-15:30 IST Equity across the Growww / NBSE platform.

## What You Are Building
A trading session orchestrator that dynamically adjusts UI order entry states, market status indicators, and margin availability based on the Indian market clock while keeping crypto spot orderbooks active 24/7.
...
## Scope Boundaries
- **In Scope:**
- Continuous 24/7/365 spot trading execution for BTC/USDT and cryptocurrency pairs.
- Scheduled market session state machine for Indian Equities: Pre-Open (09:00), Regular (09:15-15:30), Post-Close (15:40-16:00).
- Clear UI indicators displaying market open/closed status per tab with countdown timers.
- Off-market order (AMO) queuing for Indian equities while allowing instant execution for crypto spot.
- **Out of Scope / Handled Elsewhere:**
- Core orderbook matching logic.
- Blockchain consensus voting.

## Technology to Use
- Core Technologies: Go session scheduler, Quartz cron scheduler, Redis market status cache, WebSocket status broadcaster.
...
- Session Management Service, Order Gateway, UI Client Applications, Market Surveillance Engine.

## Blockchain Interaction
Blockchain settlement runs continuously 24/7, maintaining unbroken settlement availability across all calendar days.
...
1. Review system architectural boundaries and regulatory invariants for Trading Hours Harmonization: 24/7 Crypto vs 09:15-15:30 IST Equity.
2. Define the core domain entities, data models, and state lifecycle transitions for Trading Hours Harmonization: 24/7 Crypto vs 09:15-15:30 IST Equity.
...
package growww.market.session.v1;

option go_package = "growww/packages/proto/growww/market/session/v1;sessionv1";

message MarketSessionStatusRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message MarketSessionStatusResponse {
  string request_id = 1;
  bool success = 2;
...
message MarketSessionStatusEvent {
  string event_id = 1;
...
service MarketSessionService {
  rpc GetMarketSessionStatus(MarketSessionStatusRequest) returns (MarketSessionStatusResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Trading Hours Harmonization: 24/7 Crypto vs 09:15-15:30 IST Equity fully documented.
...
- **Regulatory Framework:** SEBI regulations on stock exchange trading hours, circulars on After-Market Orders (AMO).
