# 084 - Liquidation Engine, Insurance Fund & Auto-Deleveraging (ADL) Protocol

## Purpose
Protect exchange solvency and eliminate socialized losses during extreme market volatility by specifying an institutional-grade liquidation engine, capitalized Insurance Fund, and transparent Auto-Deleveraging (ADL) queue.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Liquidation Engine, Insurance Fund & Auto-Deleveraging (ADL) Protocol across the Growww / NBSE platform.

## What You Are Building
A robust risk liquidation specification detailing partial liquidation step-downs, immediate Insurance Fund absorption of bankrupt positions, and deterministic Auto-Deleveraging (ADL) ranking when the Insurance Fund is depleted.
...
## Scope Boundaries
- **In Scope:**
- Two-tier liquidation: attempts partial liquidation first (down to next leverage bracket) before complete position closure.
- Liquidated orders routed directly to orderbook matching engine as Immediate-Or-Cancel (IOC) orders.
- Exchange Insurance Fund: capitalized through liquidation penalty fees, absorbing negative balance deficits.
- Auto-Deleveraging (ADL) priority ranking: ranks profitable, highly leveraged traders to absorb bankrupt positions if Insurance Fund reaches zero.
- **Out of Scope / Handled Elsewhere:**
- Spot cash order matching.
- KYC identity document OCR.

## Technology to Use
- Core Technologies: Rust liquidation execution worker, Redis priority queue for ADL ranking, PostgreSQL liquidation audit store.
...
- Liquidation Engine, Matching Engine, Risk Engine, Insurance Fund Treasury, WebSocket Risk Stream.

## Blockchain Interaction
Claims against the Insurance Fund and ADL liquidations emit immutable proof events onto Hyperledger Besu.
...
1. Review system architectural boundaries and regulatory invariants for Liquidation Engine, Insurance Fund & Auto-Deleveraging (ADL) Protocol.
2. Define the core domain entities, data models, and state lifecycle transitions for Liquidation Engine, Insurance Fund & Auto-Deleveraging (ADL) Protocol.
...
package growww.liquidation.v1;

option go_package = "growww/packages/proto/growww/liquidation/v1;liquidationv1";

message EvaluatePositionLiquidationRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message EvaluatePositionLiquidationResponse {
  string request_id = 1;
  bool success = 2;
...
message EvaluatePositionLiquidationEvent {
  string event_id = 1;
...
service LiquidationEngineService {
  rpc EvaluatePositionLiquidation(EvaluatePositionLiquidationRequest) returns (EvaluatePositionLiquidationResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Liquidation Engine, Insurance Fund & Auto-Deleveraging (ADL) Protocol fully documented.
...
- **Regulatory Framework:** SEBI Core Settlement Guarantee Fund (SGF) default waterfall regulations, IOSCO CCP Default Management Standards.
