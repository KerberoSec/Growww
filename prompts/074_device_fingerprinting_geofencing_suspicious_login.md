# 074 - Device Fingerprinting, Geofencing & Suspicious Login Detection

## Purpose
Detect and block unauthorized account access attempts in real-time by evaluating device fingerprints, IP geolocation, impossible travel anomalies, and VPN/Tor proxy masking.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Device Fingerprinting, Geofencing & Suspicious Login Detection across the Growww / NBSE platform.

## What You Are Building
An automated risk-based authentication engine that computes unique browser and mobile device fingerprints, checks GeoIP databases, flags 'impossible travel' logins (e.g., login from Delhi followed 30 minutes later by London), and triggers security challenges.
...
## Scope Boundaries
- **In Scope:**
- Device fingerprinting: combines Canvas hash, WebGL vendor, screen resolution, user agent, and mobile hardware IDs.
- MaxMind GeoIP2 integration detecting login country, city, ISP, and proxy/VPN/Tor usage.
- Impossible travel detection: calculates geographic velocity between consecutive logins, triggering lockdown if >800 km/h.
- Automated step-up authentication: prompts mandatory email confirmation link when logging in from a new device.
- **Out of Scope / Handled Elsewhere:**
- Order matching core logic.
- On-chain gasless paymaster.

## Technology to Use
- Core Technologies: Go risk engine, MaxMind GeoIP2, FingerprintJS Pro backend wrapper, Redis login history cache, PostgreSQL security audit.
...
- Auth Service, Risk Gateway, Security Alert Daemon, User Notification Service, Mobile & Web Clients.

## Blockchain Interaction
Blocks signing of Hyperledger Besu transactions if the active user session has an unresolved suspicious login flag.
...
1. Review system architectural boundaries and regulatory invariants for Device Fingerprinting, Geofencing & Suspicious Login Detection.
2. Define the core domain entities, data models, and state lifecycle transitions for Device Fingerprinting, Geofencing & Suspicious Login Detection.
...
package growww.security.device.v1;

option go_package = "growww/packages/proto/growww/security/device/v1;devicev1";

message EvaluateLoginRiskRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message EvaluateLoginRiskResponse {
  string request_id = 1;
  bool success = 2;
...
message EvaluateLoginRiskEvent {
  string event_id = 1;
...
service DeviceSecurityService {
  rpc EvaluateLoginRisk(EvaluateLoginRiskRequest) returns (EvaluateLoginRiskResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Device Fingerprinting, Geofencing & Suspicious Login Detection fully documented.
...
- **Regulatory Framework:** SEBI Cyber Security and Cyber Resilience Framework guidelines on access control and anomaly detection.
