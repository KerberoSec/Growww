# 077 - Beneficiary Management, Address Book & Withdrawal Whitelisting

## Purpose
Prevent accidental funds loss and drainer malware exploits by providing a secure withdrawal address book requiring multi-step verification, custom label tagging, and mandatory security cool-off periods.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Beneficiary Management, Address Book & Withdrawal Whitelisting across the Growww / NBSE platform.

## What You Are Building
A beneficiary management service where users can save and label external crypto withdrawal addresses (Bitcoin, Ethereum, Tron, Solana), verify ownership via micro-deposit or cryptographic signature, and enforce a 'Whitelisted Addresses Only' mode.
...
## Scope Boundaries
- **In Scope:**
- Address book management: supports label, asset, network, destination address, and optional memo/destination tag.
- 'Whitelist Only Mode': when enabled, withdrawals are strictly prohibited to any address not in the verified whitelist.
- Mandatory 24-hour cool-off period after adding a new beneficiary address before any withdrawal can be requested.
- Beneficiary confirmation workflow: requires 2FA authentication and email confirmation link before activation.
- **Out of Scope / Handled Elsewhere:**
- Matching engine limit orders.
- Historical candlestick storage.

## Technology to Use
- Core Technologies: Go beneficiary service, Redis cool-off timer, PostgreSQL encrypted address book, Flutter Address Book UI.
...
- Wallet Service, Withdrawal Service, Security Service, Notification Service, Mobile & Web Clients.

## Blockchain Interaction
Registers verified beneficiary address hashes in the off-chain custody database linked to user Besu identity.
...
1. Review system architectural boundaries and regulatory invariants for Beneficiary Management, Address Book & Withdrawal Whitelisting.
2. Define the core domain entities, data models, and state lifecycle transitions for Beneficiary Management, Address Book & Withdrawal Whitelisting.
...
package growww.beneficiary.v1;

option go_package = "growww/packages/proto/growww/beneficiary/v1;beneficiaryv1";

message AddWithdrawalBeneficiaryRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message AddWithdrawalBeneficiaryResponse {
  string request_id = 1;
  bool success = 2;
...
message AddWithdrawalBeneficiaryEvent {
  string event_id = 1;
...
service BeneficiaryService {
  rpc AddWithdrawalBeneficiary(AddWithdrawalBeneficiaryRequest) returns (AddWithdrawalBeneficiaryResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Beneficiary Management, Address Book & Withdrawal Whitelisting fully documented.
...
- **Regulatory Framework:** FIU-IND Guidelines on Customer Due Diligence and Third-Party Withdrawal Controls, FATF Recommendation 16.
