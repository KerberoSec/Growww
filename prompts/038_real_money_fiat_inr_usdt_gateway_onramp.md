# 038 - Real-Money Fiat/INR to USDT Gateway & Instant Crypto On-Ramp

## Purpose
Enable seamless, regulatory-compliant capital onboarding for Indian traders by integrating instant INR banking rails (UPI, IMPS, NEFT) with automated USDT tokenization and liquidity conversion.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Real-Money Fiat/INR to USDT Gateway & Instant Crypto On-Ramp across the Growww / NBSE platform.

## What You Are Building
A high-availability fiat on-ramp gateway integrating Indian payment systems (UPI Autopay, Virtual Account IMPS), automated bank statement reconciliation, and instant USDT liquidity allocation.
...
## Scope Boundaries
- **In Scope:**
- Integration with NPCI UPI 2.0 and nodal escrow banking accounts (ICICI / HDFC / Axis) for instant INR deposits.
- Real-time bank webhook processing and automated matching against user Unique Reference Number (URN).
- Instant conversion of INR deposits into USDT spot balance at transparent, locked exchange rates.
- Automated anti-fraud transaction screening and velocity limits (e.g., maximum 50,000 INR per transaction for Tier 1).
- **Out of Scope / Handled Elsewhere:**
- Decentralized AMM liquidity pools.
- Hardware key zeroization.

## Technology to Use
- Core Technologies: Go payment gateway service, Nodal Bank Webhook listener, Redis lock manager, PostgreSQL double-entry fiat ledger.
...
- Fiat Gateway Service, Banking Adapter, KYC Verification Service, Wallet Service, Core Ledger Service.

## Blockchain Interaction
Mints or releases wUSDT or INR-pegged tokens on Hyperledger Besu upon verified bank settlement confirmation.
...
1. Review system architectural boundaries and regulatory invariants for Real-Money Fiat/INR to USDT Gateway & Instant Crypto On-Ramp.
2. Define the core domain entities, data models, and state lifecycle transitions for Real-Money Fiat/INR to USDT Gateway & Instant Crypto On-Ramp.
...
package growww.fiat.gateway.v1;

option go_package = "growww/packages/proto/growww/fiat/gateway/v1;gatewayv1";

message INRDepositRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
service FiatOnRampService {
  rpc InitiateINRDeposit(INRDepositRequest) returns (INRDepositResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Real-Money Fiat/INR to USDT Gateway & Instant Crypto On-Ramp fully documented.
...
- **Regulatory Framework:** RBI Guidelines on Payment Aggregators and Payment Gateways (PA/PG), PMLA rules for banking transaction records.
