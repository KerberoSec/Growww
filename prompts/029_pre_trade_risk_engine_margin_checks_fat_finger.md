# 029 - Pre-Trade Risk Engine, Margin Checks & Fat-Finger Protection

## Purpose
Protect market integrity and trader solvency by subjecting every spot order to sub-millisecond pre-trade risk validations, balance reservation checks, fat-finger price collars, and credit limits before book entry.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Pre-Trade Risk Engine, Margin Checks & Fat-Finger Protection across the Growww / NBSE platform.

## What You Are Building
A ultra-low-latency pre-trade risk engine operating in front of the matching engine that evaluates account balance sufficiency, price deviation against mark price (+/- 10% fat-finger collar), order size caps, and rate limits.
...
## Scope Boundaries
- **In Scope:**
- Sub-50 microsecond risk evaluation pipeline executing in-memory before order reaches matching engine.
- Fat-finger price collar: immediate rejection if order price deviates >10% from current volume-weighted mark price.
- Maximum notional order value check: limits single-order exposure to prevent accidental fat-finger catastrophes.
- User balance lock reservation ensuring 100% full funding for all open spot orders.
- **Out of Scope / Handled Elsewhere:**
- Post-trade clearing settlement.
- KYC identity document verification.

## Technology to Use
- Core Technologies: Rust in-memory risk checker, lock-free atomic balance maps, DPDK kernel-bypass networking, Prometheus metrics.
...
- Risk Gateway, Order Gateway, Matching Engine, Redis Cache, Market Surveillance Engine.

## Blockchain Interaction
Validates that trader wallet addresses hold verified token allocations on Hyperledger Besu before matching.
...
1. Review system architectural boundaries and regulatory invariants for Pre-Trade Risk Engine, Margin Checks & Fat-Finger Protection.
2. Define the core domain entities, data models, and state lifecycle transitions for Pre-Trade Risk Engine, Margin Checks & Fat-Finger Protection.
...
option go_package = "growww/packages/proto/growww/risk/v1;riskv1";

message ValidateOrderRiskRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ValidateOrderRiskResponse {
  string request_id = 1;
  bool success = 2;
...
message ValidateOrderRiskEvent {
  string event_id = 1;
...
service PreTradeRiskService {
  rpc ValidateOrderRisk(ValidateOrderRiskRequest) returns (ValidateOrderRiskResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Pre-Trade Risk Engine, Margin Checks & Fat-Finger Protection fully documented.
...
- **Regulatory Framework:** SEBI circulars on Stock Broker Pre-Trade Risk Controls, Basel Committee Operational Risk Standards.
