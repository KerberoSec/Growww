# 045 - Section 115BBH 30% Flat VDA Profit/Loss Realization Engine

## Purpose
Provide Indian traders with transparent, audit-ready compliance reporting by calculating taxable gains under Section 115BBH (30% flat tax on Virtual Digital Asset gains with no loss set-off) across all spot trades.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Section 115BBH 30% Flat VDA Profit/Loss Realization Engine across the Growww / NBSE platform.

## What You Are Building
A specialized tax accounting and gain calculation service implementing First-In-First-Out (FIFO) cost-basis tracking, calculating gross taxable capital gains, and generating real-time tax liability estimates.
...
## Scope Boundaries
- **In Scope:**
- FIFO (First-In, First-Out) cost-basis lot matching for all cryptocurrency acquisitions and disposals.
- Strict enforcement of Section 115BBH rules: 30% tax on gains with zero deduction for expenses (other than cost of acquisition).
- No loss set-off rule compliance: losses from one crypto trade cannot offset profits from another.
- Real-time estimated tax liability tracker on Flutter and Web portfolio screens.
- **Out of Scope / Handled Elsewhere:**
- NSE equity long-term capital gains calculation.
- Demo trading balance resets.

## Technology to Use
- Core Technologies: Go accounting service, TimescaleDB cost lot store, ClickHouse tax reporting engine, Flutter UI tax widget.
...
- Tax Accounting Service, Portfolio Service, Trade Execution Consumer, User Export Portal.

## Blockchain Interaction
Reconciles trade timestamps and prices against immutable Hyperledger Besu on-chain settlement receipts.
...
1. Review system architectural boundaries and regulatory invariants for Section 115BBH 30% Flat VDA Profit/Loss Realization Engine.
2. Define the core domain entities, data models, and state lifecycle transitions for Section 115BBH 30% Flat VDA Profit/Loss Realization Engine.
...
package growww.tax.vda.v1;

option go_package = "growww/packages/proto/growww/tax/vda/v1;vdav1";

message ComputeCapitalGainsReportRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ComputeCapitalGainsReportResponse {
  string request_id = 1;
  bool success = 2;
...
message ComputeCapitalGainsReportEvent {
  string event_id = 1;
...
service VDATaxService {
  rpc ComputeCapitalGainsReport(ComputeCapitalGainsReportRequest) returns (ComputeCapitalGainsReportResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Section 115BBH 30% Flat VDA Profit/Loss Realization Engine fully documented.
...
- **Regulatory Framework:** Section 115BBH of the Income Tax Act 1961, Finance Act 2022 provisions on Virtual Digital Assets.
