...
3. **Traceable Financial Provenance:** Every line of production code links back to a unique Prompt ID (`000` through `918`), ensuring end-to-end statutory and architectural traceability across regulatory filings, code commits, CI/CD validation pipelines, and disaster-recovery runbooks.
...
A single, uniform fee of exactly 0.00% (Zero Fee) (0.00% fee / 0 bps at launch) is charged across all traded notionals, cross-currency foreign exchange conversions, and platform transfers. Fees are calculated deterministically without tier distortion. The fee engine atomically dispatches revenues on-chain:
- 60% to the NBSE Corporate Treasury.
- 25% to the Core Settlement Guarantee Fund (SGF).
- 15% to the Investor Protection Fund (IPF).
