# 017 - Multi-Party Computation (MPC-CMP) Threshold Key Management System

## Purpose
Eliminate single points of failure in cryptocurrency custody and hot wallet signing by implementing an M-of-N Multi-Party Computation protocol (MPC-CMP) distributing cryptographic key shards across isolated cryptographic enclaves.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Multi-Party Computation (MPC-CMP) Threshold Key Management System across the Growww / NBSE platform.

## What You Are Building
An enterprise-grade MPC-CMP threshold signing custody service where private keys never exist in full memory, orchestrating Distributed Key Generation (DKG), threshold ECDSA/Ed25519 signing ceremonies, and multi-tier approval workflows.
...
## Scope Boundaries
- **In Scope:**
- 2-of-3 and 3-of-5 threshold signing topologies across User Device, Exchange Hot Node, and Backup Enclave.
- Distributed Key Generation (DKG) ceremonies generating elliptic curve keypairs without trusted dealers.
- Sub-100ms threshold signing protocol for automated hot wallet sweeping and user withdrawals.
- Key shard refresh ceremonies rotating secret shares without altering the underlying public address.
- **Out of Scope / Handled Elsewhere:**
- Cold storage paper wallet air-gapping.
- Front-end transaction broadcasting.

## Technology to Use
- Core Technologies: MPC-CMP protocol, secp256k1 & ed25519 curve libraries, HashiCorp Vault, AWS KMS / Azure Key Vault, Go / C++ cryptographic binding.
...
- MPC Custody Daemon, Wallet Service, Withdrawal Clearing Service, Audit Logger, Cold Vault Coordinator.

## Blockchain Interaction
Supports threshold transaction signing across Bitcoin Taproot/SegWit, Hyperledger Besu EVM, Tron, and Solana.
...
1. Review system architectural boundaries and regulatory invariants for Multi-Party Computation (MPC-CMP) Threshold Key Management System.
2. Define the core domain entities, data models, and state lifecycle transitions for Multi-Party Computation (MPC-CMP) Threshold Key Management System.
...
package growww.custody.mpc.v1;

option go_package = "growww/packages/proto/growww/custody/mpc/v1;mpcv1";

message RequestThresholdSignRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message RequestThresholdSignResponse {
  string request_id = 1;
  bool success = 2;
...
message RequestThresholdSignEvent {
  string event_id = 1;
...
service MPCCustodyService {
  rpc RequestThresholdSign(RequestThresholdSignRequest) returns (RequestThresholdSignResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Multi-Party Computation (MPC-CMP) Threshold Key Management System fully documented.
...
- **Regulatory Framework:** SEBI CSCRF framework guidelines on cryptographic key management, CERT-In Cyber Security Incident reporting directives.
