# 051 - Obsidian Dark Mode Palette & High-Contrast Visual Token Hierarchy

## Purpose
Deliver an addictive, visually stunning, dark-by-default design system optimized for long trading sessions, high visual clarity, and instant comprehension across mobile and web interfaces.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Obsidian Dark Mode Palette & High-Contrast Visual Token Hierarchy across the Growww / NBSE platform.

## What You Are Building
The master UI design token specification establishing the Deep Obsidian (#0B0E14) theme palette, semantic neon accents (Emerald #00F0A0 for Bullish/Profit, Crimson #FF3B56 for Bearish/Loss), and fluid contrast hierarchies.
...
## Scope Boundaries
- **In Scope:**
- Deep Obsidian background hierarchy: Canvas (#0B0E14), Surface (#121721), Card (#181F2C), Elevated (#222B3D).
- Semantic Neon color tokens: Bullish Green (#00F0A0), Bearish Red (#FF3B56), Demo Accent (#F5A623), Focus Blue (#2F80ED).
- Strict typography tokens using JetBrains Mono for numbers/prices and Inter for navigational typography.
- Cross-platform design token synchronization between Flutter (Dart theme classes) and Next.js (Tailwind CSS config).
- **Out of Scope / Handled Elsewhere:**
- Backend order matching algorithms.
- Database backup automation.

## Technology to Use
- Core Technologies: Flutter ThemeData / ColorScheme, Tailwind CSS v3 tokens, CSS variables, Style Dictionary token compiler.
...
- Flutter UI Library (packages/growww_ui), Next.js Component Library (packages/web_ui), Mobile & Web Clients.

## Blockchain Interaction
Renders real-time settlement status indicators linked to Hyperledger Besu transaction states (Pending, Confirmed, Settled).
...
1. Review system architectural boundaries and regulatory invariants for Obsidian Dark Mode Palette & High-Contrast Visual Token Hierarchy.
2. Define the core domain entities, data models, and state lifecycle transitions for Obsidian Dark Mode Palette & High-Contrast Visual Token Hierarchy.
...
package growww.ui.theme.v1;

option go_package = "growww/packages/proto/growww/ui/theme/v1;themev1";

message ActiveThemeTokensRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ActiveThemeTokensResponse {
  string request_id = 1;
  bool success = 2;
...
message ActiveThemeTokensEvent {
  string event_id = 1;
...
service ThemeTokenService {
  rpc GetActiveThemeTokens(ActiveThemeTokensRequest) returns (ActiveThemeTokensResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Obsidian Dark Mode Palette & High-Contrast Visual Token Hierarchy fully documented.
...
- **Regulatory Framework:** WCAG 2.1 Level AA Accessibility Standards for contrast ratio on dark backgrounds.
