# 058 - Push Notifications & Live Price Ticker Sticky Bar for iOS & Android

## Purpose
Keep traders continuously informed and connected to market action by implementing real-time push alerts, persistent lock-screen Live Activities (iOS), and Android Ongoing Notifications with live updating BTC/USDT tickers.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Push Notifications & Live Price Ticker Sticky Bar for iOS & Android across the Growww / NBSE platform.

## What You Are Building
A real-time push notification and mobile widget system supporting Firebase Cloud Messaging (FCM), Apple Push Notification service (APNs), iOS 17+ Dynamic Island / Live Activities, and Android persistent ongoing status-bar tickers.
...
## Scope Boundaries
- **In Scope:**
- Instant push alerts (<500ms delivery) for Order Fills, Margin Warnings, Stop-Loss triggers, and Price Alerts.
- iOS Live Activities & Dynamic Island integration displaying real-time live updating BTC/USDT price and PnL.
- Android Ongoing Notification with custom remote view updating live price ticker without waking phone CPU.
- User notification preference matrix: granular controls for marketing, security, price alerts, and trade receipts.
- **Out of Scope / Handled Elsewhere:**
- Core matching engine thread loop.
- Database disaster recovery.

## Technology to Use
- Core Technologies: Firebase Cloud Messaging (FCM), APNs HTTP/2 API, iOS ActivityKit, Android NotificationManager, Go push dispatcher.
...
- Notification Service, Market Data Engine, Order Execution Consumer, Mobile Client Apps.

## Blockchain Interaction
Dispatches push notification with transaction explorer link once trade completes on Hyperledger Besu.
...
1. Review system architectural boundaries and regulatory invariants for Push Notifications & Live Price Ticker Sticky Bar for iOS & Android.
2. Define the core domain entities, data models, and state lifecycle transitions for Push Notifications & Live Price Ticker Sticky Bar for iOS & Android.
...
option go_package = "growww/packages/proto/growww/notification/v1;notificationv1";

message SendTradeAlertRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message SendTradeAlertResponse {
  string request_id = 1;
  bool success = 2;
...
service PushNotificationService {
  rpc SendTradeAlert(SendTradeAlertRequest) returns (SendTradeAlertResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Push Notifications & Live Price Ticker Sticky Bar for iOS & Android fully documented.
...
- **Regulatory Framework:** DPDP Act 2023 user consent requirements for communications, Apple and Google Push Notification Policies.
