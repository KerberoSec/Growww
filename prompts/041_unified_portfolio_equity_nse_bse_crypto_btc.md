# 041 - Unified Portfolio Architecture: Equity (NSE/BSE) + Crypto (BTC/USDT)

## Purpose
Provide Indian investors with a single, comprehensive dashboard consolidating traditional Indian equities (NSE/BSE shares, mutual funds) alongside sovereign crypto spot assets (BTC/USDT) in real-time.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Unified Portfolio Architecture: Equity (NSE/BSE) + Crypto (BTC/USDT) across the Growww / NBSE platform.

## What You Are Building
A unified portfolio aggregation engine that ingests depository holdings (NSDL/CDSL), real-time stock quotes, and crypto spot ledger balances, computing consolidated net worth, sector allocation, and overall portfolio risk.
...
## Scope Boundaries
- **In Scope:**
- Consolidated portfolio net worth valuation in Indian Rupees (INR) updating in real-time with live exchange rates.
- Multi-asset breakdown: Equity Stocks, Sovereign Gold Bonds, Mutual Funds, Bitcoin, and Stablecoin Cash.
- Unified risk profile calculation and portfolio volatility metrics across both asset classes.
- Seamless asset filtering: toggle between All Assets, Indian Equities Only, and Crypto Spot Only.
- **Out of Scope / Handled Elsewhere:**
- Order matching engine orderbook updates.
- Hardware Security Module key generation.

## Technology to Use
- Core Technologies: Go portfolio service, TimescaleDB for historical valuations, Redis for sub-second aggregated portfolio cache.
...
- Portfolio Service, Depository Adapter, Market Data Engine, Crypto Ledger Service, Flutter Portfolio Screen.

## Blockchain Interaction
Reads verified token holdings from Hyperledger Besu and traditional Demat share holdings from depository mirrors.
...
1. Review system architectural boundaries and regulatory invariants for Unified Portfolio Architecture: Equity (NSE/BSE) + Crypto (BTC/USDT).
2. Define the core domain entities, data models, and state lifecycle transitions for Unified Portfolio Architecture: Equity (NSE/BSE) + Crypto (BTC/USDT).
...
package growww.portfolio.unified.v1;

option go_package = "growww/packages/proto/growww/portfolio/unified/v1;unifiedv1";

message ConsolidatedPortfolioRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ConsolidatedPortfolioResponse {
  string request_id = 1;
  bool success = 2;
...
message ConsolidatedPortfolioEvent {
  string event_id = 1;
...
service UnifiedPortfolioService {
  rpc GetConsolidatedPortfolio(ConsolidatedPortfolioRequest) returns (ConsolidatedPortfolioResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Unified Portfolio Architecture: Equity (NSE/BSE) + Crypto (BTC/USDT) fully documented.
...
- **Regulatory Framework:** SEBI Master Circular for Stock Brokers, PMLA norms on consolidated customer financial asset tracking.
