# 027 - Iceberg Order Slicing & Hidden Size Management

## Purpose
Enable institutional and high-net-worth crypto traders to execute large spot volume without moving the market or exposing total order size by programmatically slicing orders into visible and hidden tranches.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Iceberg Order Slicing & Hidden Size Management across the Growww / NBSE platform.

## What You Are Building
An algorithmic Iceberg execution engine that maintains total order volume off-book, publishes only a visible tranche (display size) to the public L2 orderbook, and automatically replenishes the tranche upon execution with randomized sizing.
...
## Scope Boundaries
- **In Scope:**
- Configurable display size and total order quantity with randomized variance (+/- 15%) to prevent bot detection.
- Automated tranche replenishment into L3 orderbook with new timestamp upon complete fill of current slice.
- Full balance reservation for aggregate parent order held securely in ledger balance escrow.
- Immediate cancellation capability for remaining unsliced parent quantity at any time.
- **Out of Scope / Handled Elsewhere:**
- Retail market orders.
- On-chain token minting.

## Technology to Use
- Core Technologies: Rust algorithmic order daemon, Tokio asynchronous runtime, Redis for parent order tranche tracking.
...
- Algorithmic Order Service, Matching Engine, Pre-Trade Risk Engine, Institutional Web Terminal.

## Blockchain Interaction
Individual slices settle atomically on Hyperledger Besu; parent order completion updates off-chain audit logs.
...
1. Review system architectural boundaries and regulatory invariants for Iceberg Order Slicing & Hidden Size Management.
2. Define the core domain entities, data models, and state lifecycle transitions for Iceberg Order Slicing & Hidden Size Management.
...
package growww.order.algo.v1;

option go_package = "growww/packages/proto/growww/order/algo/v1;algov1";

message IcebergOrderRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
service IcebergOrderService {
  rpc SubmitIcebergOrder(IcebergOrderRequest) returns (IcebergOrderResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Iceberg Order Slicing & Hidden Size Management fully documented.
...
- **Regulatory Framework:** SEBI institutional trade execution guidelines, MiFID II RTS 24 algorithmic order record-keeping standards.
