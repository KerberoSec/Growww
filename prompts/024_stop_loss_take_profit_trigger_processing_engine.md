# 024 - Stop-Loss & Take-Profit Trigger Processing Engine

## Purpose
Protect crypto spot traders from catastrophic market downturns and lock in trading gains by providing automated Stop-Loss and Take-Profit conditional order trigger engines with sub-millisecond activation.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Stop-Loss & Take-Profit Trigger Processing Engine across the Growww / NBSE platform.

## What You Are Building
A dedicated conditional order processing engine that monitors real-time mark and last traded prices, evaluates trigger conditions against active index prices, and automatically injects market or limit orders into the live orderbook upon crossing trigger prices.
...
## Scope Boundaries
- **In Scope:**
- Evaluation of Stop-Loss (Sell on downward cross) and Take-Profit (Sell on upward cross) conditions.
- Dual trigger price reference options: Last Traded Price (LTP) or Volume-Weighted Mark Price.
- Trigger execution latency under 5 milliseconds from price tick broadcast to active orderbook entry.
- Anti-whipsaw filter: prevents premature trigger firing during anomalous microsecond price spikes.
- **Out of Scope / Handled Elsewhere:**
- Manual limit order placement.
- Cryptographic MPC key generation.

## Technology to Use
- Core Technologies: Go conditional order daemon, Redis Sorted Sets (ZSET) for price-indexed triggers, Kafka price tick consumer.
...
- Conditional Order Service, Market Data Engine, Matching Engine, Notification Service, Risk Engine.

## Blockchain Interaction
Upon trigger firing and match, execution record is anchored onto Hyperledger Besu via atomic settlement.
...
1. Review system architectural boundaries and regulatory invariants for Stop-Loss & Take-Profit Trigger Processing Engine.
2. Define the core domain entities, data models, and state lifecycle transitions for Stop-Loss & Take-Profit Trigger Processing Engine.
...
package growww.order.conditional.v1;

option go_package = "growww/packages/proto/growww/order/conditional/v1;conditionalv1";

message RegisterTriggerOrderRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message RegisterTriggerOrderResponse {
  string request_id = 1;
  bool success = 2;
...
message RegisterTriggerOrderEvent {
  string event_id = 1;
...
service ConditionalOrderService {
  rpc RegisterTriggerOrder(RegisterTriggerOrderRequest) returns (RegisterTriggerOrderResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Stop-Loss & Take-Profit Trigger Processing Engine fully documented.
...
- **Regulatory Framework:** SEBI Investor Protection Guidelines for automated risk control mechanisms.
