# 083 - Cross-Margin & Isolated-Margin Collateral Calculator

## Purpose
Provide professional traders with flexible capital efficiency and risk management by architecting both Cross-Margin (shared collateral pool) and Isolated-Margin (ring-fenced position risk) models.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Cross-Margin & Isolated-Margin Collateral Calculator across the Growww / NBSE platform.

## What You Are Building
A mathematical margin calculation specification defining tiered leverage brackets (up to 20x), initial margin requirements (IMR), maintenance margin requirements (MMR), and real-time margin ratio health monitoring.
...
## Scope Boundaries
- **In Scope:**
- Isolated Margin: risk is strictly ring-fenced to the assigned position; liquidation cannot affect main account balance.
- Cross Margin: all available account collateral is shared across open positions to prevent premature liquidation.
- Tiered Leverage Brackets: maximum leverage decreases systematically as position notional increases (e.g., 20x up to 50k USDT; 5x over 500k USDT).
- Real-time margin call warning dispatch at 80% margin utilization; liquidation threshold at 100%.
- **Out of Scope / Handled Elsewhere:**
- Manual cash deposit processing.
- Public social media feeds.

## Technology to Use
- Core Technologies: Rust quantitative margin calculator, Redis atomic state cache, Go risk monitoring daemon, Flutter Risk Alert Sheet.
...
- Margin Engine, Risk Surveillance Daemon, Order Gateway, Notification Service, Mobile & Web Frontends.

## Blockchain Interaction
Enforces smart contract collateral lock verification on Hyperledger Besu before approving leveraged trade orders.
...
1. Review system architectural boundaries and regulatory invariants for Cross-Margin & Isolated-Margin Collateral Calculator.
2. Define the core domain entities, data models, and state lifecycle transitions for Cross-Margin & Isolated-Margin Collateral Calculator.
...
option go_package = "growww/packages/proto/growww/margin/v1;marginv1";

message ComputeAccountMarginHealthRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ComputeAccountMarginHealthResponse {
  string request_id = 1;
  bool success = 2;
...
message ComputeAccountMarginHealthEvent {
  string event_id = 1;
...
service MarginCalculationService {
  rpc ComputeAccountMarginHealth(ComputeAccountMarginHealthRequest) returns (ComputeAccountMarginHealthResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Cross-Margin & Isolated-Margin Collateral Calculator fully documented.
...
- **Regulatory Framework:** SEBI Circular on Margin Trading Facility (MTF) norms, Basel III capital adequacy principles.
