# 087 - Copy Trading Replication Engine & Master Trader Profit Sharing

## Purpose
Provide retail investors with access to top-performing crypto strategies while monetizing experienced traders by implementing a high-throughput copy trading engine with proportional order replication and profit sharing.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Copy Trading Replication Engine & Master Trader Profit Sharing across the Growww / NBSE platform.

## What You Are Building
An event-driven copy trading replication service that intercepts master trader orders, replicates them proportionally across follower accounts within 50 milliseconds, and distributes weekly performance fees (10-20% high-water mark).
...
## Scope Boundaries
- **In Scope:**
- Proportional position sizing: follower trade size = master_order_size * (follower_equity / master_equity).
- Sub-50ms execution replication latency preventing front-running and follower slippage.
- High-Water Mark (HWM) profit sharing engine: automatically deducts 10-20% performance fee from follower profits.
- Follower risk controls: customizable maximum allocation per master, daily stop-loss limit, and slippage protection.
- **Out of Scope / Handled Elsewhere:**
- Manual uncollateralized lending.
- Blockchain validator consensus voting.

## Technology to Use
- Core Technologies: Go copy trading orchestrator, Redis follower registry, Kafka master trade stream, PostgreSQL profit ledger.
...
- Copy Trading Service, Order Gateway, Matching Engine, Notification Service, Mobile & Web Leaderboards.

## Blockchain Interaction
Records profit-sharing fee deductions and master trader distributions onto Hyperledger Besu ledger.
...
1. Review system architectural boundaries and regulatory invariants for Copy Trading Replication Engine & Master Trader Profit Sharing.
2. Define the core domain entities, data models, and state lifecycle transitions for Copy Trading Replication Engine & Master Trader Profit Sharing.
...
option go_package = "growww/packages/proto/growww/copytrading/v1;copytradingv1";

message ReplicateMasterTradeRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ReplicateMasterTradeResponse {
  string request_id = 1;
  bool success = 2;
...
message ReplicateMasterTradeEvent {
  string event_id = 1;
...
service CopyTradingService {
  rpc ReplicateMasterTrade(ReplicateMasterTradeRequest) returns (ReplicateMasterTradeResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Copy Trading Replication Engine & Master Trader Profit Sharing fully documented.
...
- **Regulatory Framework:** SEBI Investment Advisers Regulations (RA/IA norms), ASCI Disclosures on Social and Strategy Trading.
