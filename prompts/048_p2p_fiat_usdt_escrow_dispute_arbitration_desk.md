# 048 - P2P Fiat-to-USDT Escrow Smart Contract & Dispute Arbitration Desk

## Purpose
Provide Indian traders with a secure, zero-slippage peer-to-peer on-ramp for converting Indian Rupees directly into USDT via a trust-minimized smart contract escrow and institutional maker-checker dispute desk.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for P2P Fiat-to-USDT Escrow Smart Contract & Dispute Arbitration Desk across the Growww / NBSE platform.

## What You Are Building
A peer-to-peer fiat trading platform featuring automated smart contract token locking, real-time encrypted trader chat, UPI payment reference verification, and a multi-tiered administrative dispute resolution desk.
...
## Scope Boundaries
- **In Scope:**
- Smart contract escrow on Hyperledger Besu locking seller USDT upon trade initiation.
- Encrypted end-to-end buyer-seller chat for sharing UPI IDs and payment confirmation screenshots.
- Automated escrow release upon seller digital signature confirming receipt of bank funds.
- Maker-checker dispute arbitration portal requiring banking statement verification to resolve contested trades.
- **Out of Scope / Handled Elsewhere:**
- Automated L3 orderbook matching.
- Algorithmic market making.

## Technology to Use
- Core Technologies: Solidity P2PEscrow.sol contract, Go P2P messaging daemon, WebRTC encrypted chat, Back-Office Admin Portal.
...
- P2P Service, Besu Escrow Contract, Chat Service, Admin Dispute Desk, Notification Service.

## Blockchain Interaction
Hyperledger Besu P2PEscrow.sol locking and transferring USDT tokens between buyer and seller public addresses.
...
1. Review system architectural boundaries and regulatory invariants for P2P Fiat-to-USDT Escrow Smart Contract & Dispute Arbitration Desk.
2. Define the core domain entities, data models, and state lifecycle transitions for P2P Fiat-to-USDT Escrow Smart Contract & Dispute Arbitration Desk.
...
option go_package = "growww/packages/proto/growww/p2p/v1;p2pv1";

message P2PTradeRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
service P2PEscrowService {
  rpc InitiateP2PTrade(P2PTradeRequest) returns (P2PTradeResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for P2P Fiat-to-USDT Escrow Smart Contract & Dispute Arbitration Desk fully documented.
...
- **Regulatory Framework:** PMLA Customer Due Diligence Guidelines, Indian Contract Act 1872 on valid electronic escrow agreements.
