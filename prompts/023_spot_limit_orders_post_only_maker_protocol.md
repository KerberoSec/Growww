# 023 - Spot Limit Orders & Post-Only Maker Execution Protocol

## Purpose
Provide professional and algorithmic market makers with deterministic limit order placement, cancellation, and Maker-guarantee flags (Post-Only) to supply depth liquidity without incurring taker fees.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Spot Limit Orders & Post-Only Maker Execution Protocol across the Growww / NBSE platform.

## What You Are Building
A high-throughput spot limit order management module supporting standard Good-Til-Cancelled (GTC), Immediate-Or-Cancel (IOC), Fill-Or-Kill (FOK), and Post-Only maker execution protocols.
...
## Scope Boundaries
- **In Scope:**
- Post-Only flag enforcement: order automatically cancels if it would immediately cross the spread and take liquidity.
- Support for GTC, IOC, and FOK time-in-force execution instructions.
- Order amendment in-flight: quantity reduction preserves book priority; price change re-enters at tail.
- Sub-50 microsecond order placement, modification, and cancellation handling.
- **Out of Scope / Handled Elsewhere:**
- Stop-loss condition evaluation.
- Bank wire settlement.

## Technology to Use
- Core Technologies: Rust matching engine, Go order gateway, FlatBuffers for internal IPC, low-latency epoll network listener.
...
- Order Gateway Service, Matching Engine, Risk Engine, Account Balance Daemon, WebSocket Broadcaster.

## Blockchain Interaction
Order placement locks tokens off-chain; fills submit settlement records to Hyperledger Besu DvP contracts.
...
1. Review system architectural boundaries and regulatory invariants for Spot Limit Orders & Post-Only Maker Execution Protocol.
2. Define the core domain entities, data models, and state lifecycle transitions for Spot Limit Orders & Post-Only Maker Execution Protocol.
...
package growww.order.limit.v1;

option go_package = "growww/packages/proto/growww/order/limit/v1;limitv1";
...
service LimitOrderService {
  rpc SubmitLimitOrder(LimitOrderRequest) returns (LimitOrderResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Spot Limit Orders & Post-Only Maker Execution Protocol fully documented.
...
- **Regulatory Framework:** SEBI Market Maker incentive and fee disclosure guidelines, US SEC Regulation NMS Rule 610 principles.
