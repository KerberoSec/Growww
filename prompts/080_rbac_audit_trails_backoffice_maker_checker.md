# 080 - Role-Based Access Control (RBAC) & Audit Trails for Back-Office Staff

## Purpose
Prevent rogue employee abuse, insider fraud, and operational errors by enforcing granular Role-Based Access Control (RBAC), mandatory Maker-Checker approval quorums, and immutable administrative audit trails.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Role-Based Access Control (RBAC) & Audit Trails for Back-Office Staff across the Growww / NBSE platform.

## What You Are Building
An enterprise administrative governance framework governing back-office operators, support agents, and compliance officers, featuring fine-grained permission matrices, dual-authorization (Maker-Checker) for sensitive actions, and WORM logging.
...
## Scope Boundaries
- **In Scope:**
- Fine-grained RBAC permissions: Support Tier 1 (View only), Compliance (KYC review), Finance (Approve withdrawal), Superadmin.
- Maker-Checker dual authorization: any manual fund adjustment or parameter change requires two independent staff approvals.
- Tamper-proof administrative audit trail: every staff click, view, and modification logged with IP, timestamp, and action diff.
- Automatic session lock after 10 minutes of inactivity and mandatory FIDO2 hardware token authentication for all staff.
- **Out of Scope / Handled Elsewhere:**
- Public user order entry.
- Automated market making algorithms.

## Technology to Use
- Core Technologies: Go Admin RBAC service, Casbin authorization library, AWS S3 Object Lock (WORM storage), React Admin Dashboard.
...
- Admin Portal, Auth Service, Compliance Service, Financial Operations Service, Security Audit Store.

## Blockchain Interaction
Multi-signature administrative operations on Hyperledger Besu require M-of-N hardware signatures from authorized staff.
...
1. Review system architectural boundaries and regulatory invariants for Role-Based Access Control (RBAC) & Audit Trails for Back-Office Staff.
2. Define the core domain entities, data models, and state lifecycle transitions for Role-Based Access Control (RBAC) & Audit Trails for Back-Office Staff.
...
package growww.admin.rbac.v1;

option go_package = "growww/packages/proto/growww/admin/rbac/v1;rbacv1";

message MakerCheckerActionRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message MakerCheckerActionResponse {
  string request_id = 1;
  bool success = 2;
...
message MakerCheckerActionEvent {
  string event_id = 1;
...
service AdminRBACService {
  rpc SubmitMakerCheckerAction(MakerCheckerActionRequest) returns (MakerCheckerActionResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Role-Based Access Control (RBAC) & Audit Trails for Back-Office Staff fully documented.
...
- **Regulatory Framework:** SEBI Cyber Security Framework Section on User Access Control, ISO/IEC 27001 Annex A.9 Access Control.
