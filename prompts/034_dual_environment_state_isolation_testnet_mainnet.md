# 034 - Dual-Environment State Isolation (Demo Testnet vs Real Besu Mainnet)

## Purpose
Guarantee zero accidental leakage, cross-contamination, or state corruption between simulated paper trading and real-money financial operations by engineering strict physical and logical infrastructure isolation.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Dual-Environment State Isolation (Demo Testnet vs Real Besu Mainnet) across the Growww / NBSE platform.

## What You Are Building
A robust dual-environment isolation architecture encompassing separate database schemas, independent Kafka topics, distinct Redis clusters, and separate Hyperledger Besu network IDs (Testnet vs Mainnet).
...
## Scope Boundaries
- **In Scope:**
- Logical and network segmentation: Demo services run on isolated Kubernetes namespaces and databases.
- Distinct Hyperledger Besu networks: Chain ID 1337 (Testnet) vs Chain ID 2026 (Mainnet).
- Strict API Gateway routing: demo requests carry explicit X-Environment: DEMO header and distinct JWT scopes.
- Data leakage prevention: real user KYC and banking data are never accessible to demo microservices.
- **Out of Scope / Handled Elsewhere:**
- Cross-chain atomic bridges.
- Options pricing algorithms.

## Technology to Use
- Core Technologies: Kubernetes Network Policies, Envoy API Gateway filter, PostgreSQL role-based schema isolation, Kafka ACLs.
...
- API Gateway, Demo Services Namespace, Production Services Namespace, Besu Testnet Nodes, Besu Mainnet Nodes.

## Blockchain Interaction
Separates Besu Testnet smart contracts from immutable Besu Mainnet DvP settlement contracts.
...
1. Review system architectural boundaries and regulatory invariants for Dual-Environment State Isolation (Demo Testnet vs Real Besu Mainnet).
2. Define the core domain entities, data models, and state lifecycle transitions for Dual-Environment State Isolation (Demo Testnet vs Real Besu Mainnet).
...
package growww.environment.v1;

option go_package = "growww/packages/proto/growww/environment/v1;environmentv1";

message ValidateEnvironmentRouteRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message ValidateEnvironmentRouteResponse {
  string request_id = 1;
  bool success = 2;
...
message ValidateEnvironmentRouteEvent {
  string event_id = 1;
...
service EnvironmentRouterService {
  rpc ValidateEnvironmentRoute(ValidateEnvironmentRouteRequest) returns (ValidateEnvironmentRouteResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Dual-Environment State Isolation (Demo Testnet vs Real Besu Mainnet) fully documented.
...
- **Regulatory Framework:** SEBI Cyber Security and Resilience Framework (CSCRF) Environment Separation Guidelines, ISO 27001.
