# 071 - User Profile, Security Settings & Session Management Specification

## Purpose
Empower users to manage their personal profile, active device sessions, notification preferences, and account security controls through an intuitive, dark-mode settings center.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for User Profile, Security Settings & Session Management Specification across the Growww / NBSE platform.

## What You Are Building
A comprehensive user settings and session management specification governing profile metadata, active login sessions, remote session revocation, password changes, and account closure requests.
...
## Scope Boundaries
- **In Scope:**
- Unified Profile Center: manages avatar, display name, registered email, phone number, and verified KYC status.
- Active Sessions Manager: lists all logged-in devices with IP address, location, browser/device model, and last active time.
- Instant Remote Revocation: one-tap 'Log Out All Other Devices' button invalidating refresh tokens in Redis.
- Account closure and self-exclusion workflow allowing users to freeze trading activities voluntarily.
- **Out of Scope / Handled Elsewhere:**
- Order matching core engine.
- Market data aggregation.

## Technology to Use
- Core Technologies: Go User Service, Redis session token store, PostgreSQL user database, Flutter Settings UI, Next.js Profile Page.
...
- User Service, Auth Gateway, Session Store, Notification Service, Client Application Settings Screens.

## Blockchain Interaction
Binds user identity cryptographically to their registered Hyperledger Besu trader address and MPC public key.
...
1. Review system architectural boundaries and regulatory invariants for User Profile, Security Settings & Session Management Specification.
2. Define the core domain entities, data models, and state lifecycle transitions for User Profile, Security Settings & Session Management Specification.
...
package growww.user.settings.v1;

option go_package = "growww/packages/proto/growww/user/settings/v1;settingsv1";
...
## Acceptance Criteria
- [ ] Core specification and domain data models for User Profile, Security Settings & Session Management Specification fully documented.
...
- **Regulatory Framework:** DPDP Act 2023 Section 6 (Notice and Consent Management), SEBI Client Code Modification (UCC) guidelines.
