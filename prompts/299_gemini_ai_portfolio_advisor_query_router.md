# 299 - Gemini AI Portfolio Advisor Context Builder & Query Router

## Purpose
Context-aware query engine retrieving sanitized, zero-PII portfolio metrics and passing them to Gemini LLM for analysis.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Gemini AI Portfolio Advisor Context Builder & Query Router across the Growww / NBSE platform.

## What You Are Building
A production-grade component specification for Gemini AI Portfolio Advisor Context Builder & Query Router within Category 2: Backend Microservices.
...
1. Review system architectural boundaries and formal specifications for Gemini AI Portfolio Advisor Context Builder & Query Router.
...
package growww.gemini.v1;

option go_package = "growww/packages/proto/growww/gemini/v1;geminiv1";

message GeminiaiportfolioadvisorqueryrouterRequest {
  string request_id = 1;
  string entity_id = 2;
  uint64 amount_e8 = 3;
...
message GeminiaiportfolioadvisorqueryrouterResponse {
  string request_id = 1;
  bool success = 2;
...
service GeminiaiportfolioadvisorqueryrouterService {
  rpc Execute(GeminiaiportfolioadvisorqueryrouterRequest) returns (GeminiaiportfolioadvisorqueryrouterResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Gemini AI Portfolio Advisor Context Builder & Query Router fully documented.
