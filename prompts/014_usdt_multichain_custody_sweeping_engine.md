# 014 - USDT Multichain Rails (ERC-20, TRC-20, Polygon, Solana) Custody & Sweeping

## Purpose
Deliver seamless, low-cost Tether (USDT) liquidity for crypto spot trading pairs by engineering a multi-chain deposit, confirmation, and automated treasury sweeping pipeline across Ethereum, Tron, Polygon, and Solana.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for USDT Multichain Rails (ERC-20, TRC-20, Polygon, Solana) Custody & Sweeping across the Growww / NBSE platform.

## What You Are Building
A high-performance multichain USDT ingestion and hot-to-cold vault sweeping engine with automated gas station top-ups, transaction re-org resilience, and real-time ledger balance crediting.
...
## Scope Boundaries
- **In Scope:**
- Multichain deposit address allocation and smart contract event listener (ERC-20 Transfer events, Tron TRC-20, Solana SPL Token).
- Automated gas provisioning (ETH/TRX/POL/SOL) for sweep transactions from deposit addresses to omnibus hot vaults.
- Confirmation matrix: Ethereum (12 blocks), Tron (19 blocks), Polygon (128 blocks), Solana (32 confirmed slots).
- Double-spend and re-organization detection with automatic credit rollback before trading.
- **Out of Scope / Handled Elsewhere:**
- P2P fiat INR escrow settlement.
- Orderbook matching algorithm.

## Technology to Use
- Core Technologies: EVM JSON-RPC, TronGrid API, Solana JSON-RPC / WebSocket, Go concurrency workers, Redis lock manager, Kafka event publisher.
...
- Multichain Deposit Service, Sweeper Daemon, Treasury Management Service, Spot Wallet Ledger, Kafka Stream.

## Blockchain Interaction
Multi-chain integration covering Ethereum ERC-20, Tron TRC-20, Polygon POS ERC-20, and Solana SPL USDT, bridging into Hyperledger Besu wUSDT.
...
1. Review system architectural boundaries and regulatory invariants for USDT Multichain Rails (ERC-20, TRC-20, Polygon, Solana) Custody & Sweeping.
2. Define the core domain entities, data models, and state lifecycle transitions for USDT Multichain Rails (ERC-20, TRC-20, Polygon, Solana) Custody & Sweeping.
...
package growww.usdt.v1;

option go_package = "growww/packages/proto/growww/usdt/v1;usdtv1";
...
service USDTIngressService {
  rpc NotifyDeposit(NotifyDepositRequest) returns (NotifyDepositResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for USDT Multichain Rails (ERC-20, TRC-20, Polygon, Solana) Custody & Sweeping fully documented.
...
- **Regulatory Framework:** FIU-IND circulars on foreign crypto asset transfers, PMLA Section 12 maintenance of audit records for all multi-chain stablecoin deposits.
