# 037 - Demo-to-Real Trading Onboarding & Graduation Funnel

## Purpose
Provide a guided, confidence-building pathway for users who master paper trading to transition safely into real-money spot crypto trading with automated risk assessment and step-by-step KYC onboarding.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Demo-to-Real Trading Onboarding & Graduation Funnel across the Growww / NBSE platform.

## What You Are Building
A graduation funnel orchestration service that assesses a demo trader's risk discipline, prompts them with customized educational feedback, and guides them into live KYC, bank linkage, and real-money trading.
...
## Scope Boundaries
- **In Scope:**
- Automated readiness scoring based on demo trading history (e.g., minimum 10 trades, positive risk-reward ratio).
- Seamless one-tap transition into KYC onboarding flow pre-filling verified user preferences.
- Initial real-trade safety restrictions: optional recommended maximum position sizing for the first 7 days.
- Interactive risk acknowledgement modal ensuring users understand real capital risks.
- **Out of Scope / Handled Elsewhere:**
- Order matching engine algorithms.
- High-frequency market making.

## Technology to Use
- Core Technologies: Go graduation workflow service, Temporal.io state orchestrator, User Service, Notification Service, Flutter UI.
...
- Graduation Service, User Service, KYC Verification Engine, Risk Profiling Daemon, Client Applications.

## Blockchain Interaction
Marks the user profile as graduated and provisions live deposit addresses across Besu, Bitcoin, and USDT rails.
...
1. Review system architectural boundaries and regulatory invariants for Demo-to-Real Trading Onboarding & Graduation Funnel.
2. Define the core domain entities, data models, and state lifecycle transitions for Demo-to-Real Trading Onboarding & Graduation Funnel.
...
package growww.funnel.v1;

option go_package = "growww/packages/proto/growww/funnel/v1;funnelv1";

message EvaluateGraduationReadinessRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message EvaluateGraduationReadinessResponse {
  string request_id = 1;
  bool success = 2;
...
message EvaluateGraduationReadinessEvent {
  string event_id = 1;
...
service GraduationFunnelService {
  rpc EvaluateGraduationReadiness(EvaluateGraduationReadinessRequest) returns (EvaluateGraduationReadinessResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Demo-to-Real Trading Onboarding & Graduation Funnel fully documented.
...
- **Regulatory Framework:** SEBI Suitability and Appropriateness Norms for retail investors entering volatile financial markets.
