# 036 - Demo Trade Analytics, PnL Leaderboard & Gamification Engine

## Purpose
Drive user engagement, trading education, and platform retention by calculating real-time paper trading performance metrics, win-rate analytics, sharpe ratios, and competitive community leaderboards.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Demo Trade Analytics, PnL Leaderboard & Gamification Engine across the Growww / NBSE platform.

## What You Are Building
An event-driven analytics and gamification engine that tracks demo trade executions, calculates daily and monthly Return on Investment (ROI), and ranks traders on a public, privacy-preserving leaderboard.
...
## Scope Boundaries
- **In Scope:**
- Real-time calculation of demo portfolio PnL, win/loss ratio, maximum drawdown, and Sharpe ratio.
- Daily, weekly, and all-time demo trader leaderboard with pseudonymous handles.
- Milestone achievement system awarding badges for first trade, 5-day profit streak, and disciplined stop-loss usage.
- Performance export feature allowing users to review their trading history and identify emotional mistakes.
- **Out of Scope / Handled Elsewhere:**
- Real-money profit payouts.
- Crypto tax filing reports.

## Technology to Use
- Core Technologies: Apache Flink stream analytics, ClickHouse historical metrics store, Redis Leaderboard (ZSET), Go ranking daemon.
...
- Demo Analytics Service, Leaderboard Service, Notification Service, Mobile Dashboard, Web Leaderboard Page.

## Blockchain Interaction
Optionally publishes weekly leaderboard root hashes onto Besu Testnet for immutable contest verification.
...
1. Review system architectural boundaries and regulatory invariants for Demo Trade Analytics, PnL Leaderboard & Gamification Engine.
2. Define the core domain entities, data models, and state lifecycle transitions for Demo Trade Analytics, PnL Leaderboard & Gamification Engine.
...
package growww.demo.analytics.v1;

option go_package = "growww/packages/proto/growww/demo/analytics/v1;analyticsv1";
...
service DemoAnalyticsService {
  rpc GetLeaderboardRank(LeaderboardRankRequest) returns (LeaderboardRankResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Demo Trade Analytics, PnL Leaderboard & Gamification Engine fully documented.
...
- **Regulatory Framework:** ASCI Guidelines on financial gaming and simulation disclosures, SEBI Investor Education mandates.
