# 078 - Sub-Accounts & Institutional Master/Trading Account Hierarchy

## Purpose
Enable corporate entities, asset managers, and algorithmic trading teams to manage multiple distinct trading strategies under a single unified master account with customized access controls and fund routing.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Sub-Accounts & Institutional Master/Trading Account Hierarchy across the Growww / NBSE platform.

## What You Are Building
An enterprise sub-account hierarchy architecture supporting up to 100 isolated sub-accounts per master entity, independent API key generation, zero-fee internal balance transfers, and role-based permissions.
...
## Scope Boundaries
- **In Scope:**
- Master Account dashboard: view aggregate balances, PnL, and activity across all child trading accounts.
- Sub-account isolation: independent API keys, orderbooks, and open orders with zero state bleed between accounts.
- Instant internal transfers: zero-latency, zero-fee asset transfers between master and sub-accounts.
- Role assignment: read-only analyst, trading-only bot, and full administrative manager permissions.
- **Out of Scope / Handled Elsewhere:**
- Public marketing websites.
- Consumer retail referrals.

## Technology to Use
- Core Technologies: Go enterprise account service, PostgreSQL hierarchical tree (ltree extension), Redis account cache, Web Institutional Portal.
...
- Institutional Service, User Service, Order Service, Ledger Service, Institutional Admin Console.

## Blockchain Interaction
Maintains sub-account balance sub-ledgers on Hyperledger Besu with master account umbrella custody.
...
1. Review system architectural boundaries and regulatory invariants for Sub-Accounts & Institutional Master/Trading Account Hierarchy.
2. Define the core domain entities, data models, and state lifecycle transitions for Sub-Accounts & Institutional Master/Trading Account Hierarchy.
...
package growww.account.subaccount.v1;

option go_package = "growww/packages/proto/growww/account/subaccount/v1;subaccountv1";

message TransferSubAccountFundsRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message TransferSubAccountFundsResponse {
  string request_id = 1;
  bool success = 2;
...
message TransferSubAccountFundsEvent {
  string event_id = 1;
...
service SubAccountService {
  rpc TransferSubAccountFunds(TransferSubAccountFundsRequest) returns (TransferSubAccountFundsResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Sub-Accounts & Institutional Master/Trading Account Hierarchy fully documented.
...
- **Regulatory Framework:** SEBI guidelines on Institutional Client Code allocation and Corporate Account Governance.
