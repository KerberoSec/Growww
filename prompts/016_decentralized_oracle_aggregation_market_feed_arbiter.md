# 016 - Decentralized Oracle Aggregation (Chainlink + Pyth + Binance Feed Arbiter)

## Purpose
Guarantee unshakeable, manipulation-resistant pricing for spot reference rates, mark prices, and demo paper trading feeds by aggregating on-chain decentralized oracles and Tier-1 exchange market data feeds with outlier filtering.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Decentralized Oracle Aggregation (Chainlink + Pyth + Binance Feed Arbiter) across the Growww / NBSE platform.

## What You Are Building
A robust Oracle Aggregator and Feed Arbiter service that combines Chainlink EVM data feeds, Pyth Network pull oracles, Binance real-time WebSocket feeds, and Coinbase Pro prices, computing a volume-weighted median price with deviation circuit breakers.
...
## Scope Boundaries
- **In Scope:**
- Integration with Chainlink Price Feeds (BTC/USD, USDT/USD) and Pyth Network low-latency benchmarks.
- Real-time WebSocket normalization for Binance, OKX, and Coinbase BTC/USDT spot prices.
- Fault-tolerant Medianizer algorithm eliminating stale or divergent feeds (>0.5% deviation threshold).
- Automatic fallback cascade: Primary Pyth -> Secondary Chainlink -> Tertiary Exchange Median.
- **Out of Scope / Handled Elsewhere:**
- Internal matching engine order execution.
- User order entry validation.

## Technology to Use
- Core Technologies: Pyth Network Go SDK, Chainlink EVM contract interfaces, Gorilla WebSocket, Redis for sub-millisecond price cache, Prometheus metrics.
...
- Oracle Aggregator Service, Market Data Service, Pre-Trade Risk Engine, Demo Trading Engine, Besu Oracle Anchor Contract.

## Blockchain Interaction
Hyperledger Besu OnChainOracle.sol smart contract receiving heartbeat updates every 2 seconds with cryptographic signature validation.
...
1. Review system architectural boundaries and regulatory invariants for Decentralized Oracle Aggregation (Chainlink + Pyth + Binance Feed Arbiter).
2. Define the core domain entities, data models, and state lifecycle transitions for Decentralized Oracle Aggregation (Chainlink + Pyth + Binance Feed Arbiter).
...
package growww.oracle.v1;

option go_package = "growww/packages/proto/growww/oracle/v1;oraclev1";
...
service OracleAggregatorService {
  rpc GetMarkPrice(MarkPriceRequest) returns (MarkPriceResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Decentralized Oracle Aggregation (Chainlink + Pyth + Binance Feed Arbiter) fully documented.
...
- **Regulatory Framework:** SEBI Reference Price calculation standards, IOSCO Principles for Financial Benchmarks.
