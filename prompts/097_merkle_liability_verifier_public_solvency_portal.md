# 097 - Merkle Tree Liability Verifier & Public Solvency Proof Portal

## Purpose
Deliver radical financial transparency by providing a public, browser-based Merkle Tree Solvency Verifier where any trader can independently confirm their account balance inclusion in the exchange's audited liabilities.

This document establishes the authoritative technical blueprint, mathematical models, state machines, API interfaces, failure recovery mechanisms, and regulatory compliance mapping for Merkle Tree Liability Verifier & Public Solvency Proof Portal across the Growww / NBSE platform.

## What You Are Building
A zero-knowledge, privacy-preserving public web portal and cryptographic verification library allowing users to paste their anonymized Account Hash and verify that their full balance is accounted for in the published Merkle tree.
...
## Scope Boundaries
- **In Scope:**
- Browser-based cryptographic verification using WebAssembly (Wasm) and SHA-256 / Poseidon hashing.
- Privacy preservation: users verify their balance inclusion without revealing their identity or balance to other users.
- Public cryptographic proof downloads: publishes full anonymized Merkle tree root and auditor attestation report hourly.
- Automated comparison against on-chain Proof of Reserve wallet addresses displaying real-time solvency ratio (>100%).
- **Out of Scope / Handled Elsewhere:**
- Internal database credential management.
- Private customer KYC file viewing.

## Technology to Use
- Core Technologies: Next.js Web Portal, Rust compiled to WebAssembly (Wasm), Sparse Merkle Tree verifier, Tailwind CSS Dark Theme.
...
- Public Solvency Portal, Proof of Reserve Daemon, Audit Node, Hyperledger Besu Explorer, Marketing Site.

## Blockchain Interaction
Pulls the authoritative root hash and auditor signature directly from Hyperledger Besu SolvencyRegistry.sol contract.
...
1. Review system architectural boundaries and regulatory invariants for Merkle Tree Liability Verifier & Public Solvency Proof Portal.
2. Define the core domain entities, data models, and state lifecycle transitions for Merkle Tree Liability Verifier & Public Solvency Proof Portal.
...
package growww.solvency.portal.v1;

option go_package = "growww/packages/proto/growww/solvency/portal/v1;portalv1";

message AccountInclusionRequest {
  string request_id = 1;
  string user_id = 2;
  string symbol = 3;
  uint64 amount_e8 = 4;
...
message AccountInclusionResponse {
  string request_id = 1;
  bool success = 2;
...
service PublicSolvencyService {
  rpc VerifyAccountInclusion(AccountInclusionRequest) returns (AccountInclusionResponse);
}
```
...
## Acceptance Criteria
- [ ] Core specification and domain data models for Merkle Tree Liability Verifier & Public Solvency Proof Portal fully documented.
...
- **Regulatory Framework:** SEBI guidelines on Public Disclosure of Financial Health, AICPA SOC 1/2 Trust Services Criteria for Solvency.
