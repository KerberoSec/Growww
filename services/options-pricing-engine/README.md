...
## Executive Overview
`options-pricing-engine` is an institutional-grade microservice written in **Rust** within the Growww / NBSE trading platform architecture.

## Primary Responsibility
Black-Scholes quantitative engine computing option Greeks (Delta, Gamma, Vega, Theta) and IV surfaces.

## Interface & Communication Boundaries
- **Primary gRPC Service**: `OptionsPricingEngineService` (defined in `packages/proto/`)
- **Inbound Event Stream**: `options.quotes.v1`
- **Outbound Event Stream**: `options.greeks.v1`
