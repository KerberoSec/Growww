# custodian-depository-adapter

## Executive Overview
`custodian-depository-adapter` is an institutional-grade microservice written in **Go** within the Growww / NBSE trading platform architecture.

## Primary Responsibility
Depository interface connecting NSDL and CDSL for securities delivery instruction slips (DIS).

## Interface & Communication Boundaries
- **Primary gRPC Service**: `DepositoryAdapterService` (defined in `packages/proto/`)
- **Inbound Event Stream**: `depository.dis.v1`
- **Outbound Event Stream**: `depository.status.v1`
